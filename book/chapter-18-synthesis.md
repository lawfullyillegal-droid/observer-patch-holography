...
| Quark exact witness | Exact same-family `(u, d, s, c, b, t)` sextet on the selected `sigma_ref` sheet. | Same-family-only. The present premise set carries a no-go for theorem-grade quark closure; the exact next objects are the minimal extension triple `H_mass : ell_ud = log(c_d / c_u)`, `H_phys : s_ud^phys : M_ud^{CR,phys} -> Sigma_ud^phys`, and `H_abs : A_q^phys : Sigma_ud^phys -> R`; a continuation-only D12 internal backread sidecar fixes the mass-side scalar package numerically without removing the wrong CKM branch. |
