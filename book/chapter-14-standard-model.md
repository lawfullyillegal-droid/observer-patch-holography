...
| Quark exact witness | Exact same-family `(u, d, s, c, b, t)` sextet on the selected `sigma_ref` sheet | Same-family-only. The present premise set carries a no-go for full physical quark closure, and the exact next objects are the minimal extension triple `H_mass : ell_ud = log(c_d / c_u)`, `H_phys : s_ud^phys : M_ud^{CR,phys} -> Sigma_ud^phys`, and `H_abs : A_q^phys : Sigma_ud^phys -> R`; a continuation-only D12 internal backread sidecar fixes the mass-side scalar package numerically without removing the wrong CKM branch |
...
**The open frontier.** The remaining hard work is finishing the charged-lepton scale, computing the minimal quark extension triple `(H_mass, H_phys, H_abs)` after the negative local `sigma_ref` selector closure, and completing the backend-bundle plus nonperturbative strong-dynamics program only if hadron closure returns to scope.
