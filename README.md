...
- **Still unfinished:** the broader quark family remains continuation-level and the current quark sheet is a wrong-branch no-go for the physical CKM shell, so the next exact quark object is the finite same-label left-handed orbit together with the provider/evaluator that exposes the first non-empty orbit elements before any selected-branch mass-scale law; the charged-lepton lane has its centered shape closed but its absolute normalization still open because the live frontier is two-stage, with `C_hat_e^{cand}` still waiting on branch-generator splitting and the absolute side then still waiting on a refinement-stable uncentered trace lift beneath the determinant-line route; the current weighted-cycle neutrino branch matches the physical PMNS/hierarchy pattern and closes the normalized overlap-defect weight section, but the absolute attachment still waits on the residual bridge scalar `B_nu`, which then induces `lambda_nu` after the exact `q_mean^p_nu` factorization; and hadrons are execution-bound because promotable rows require real nonperturbative production computation and systematics rather than theorem-only completion.
...
- Neutrino solar splitting `Δm21²`: compare-only absolute splitting; residual bridge law `B_nu` is still open.
- Neutrino atmospheric splitting `Δm32²`: compare-only atmospheric anchor used to place the branch on an eV scale.
- Neutrino normal-order splitting `Δm31²`: compare-only absolute splitting; residual bridge law `B_nu` is still open.
...
**Deriving the Particle Zoo from Observer Consistency** is the particle-spectrum status paper. It records the closed target-free electroweak \(W/Z\) theorem after fixing the shared calibration scale \(P\), tracks the current Higgs/top and quark outputs, states explicitly that the current quark sheet is transport-closed but wrong-branch and therefore waits on the finite same-label left-handed orbit together with the provider/evaluator that exposes the first non-empty orbit elements before any selected-branch mass-scale law, states explicitly that the charged-lepton lane fixes only the centered log class until `C_hat_e^{cand}` is promoted by branch-generator splitting and the downstream refinement-stable uncentered trace lift beneath the determinant-line route is completed, records the current weighted-cycle neutrino branch with `theta12 = 34.2259°`, `theta23 = 49.7228°`, `theta13 = 8.68636°`, `delta = 305.581°`, and `Delta m21^2 / Delta m32^2 = 0.03072111`, compares those numbers directly against the PDG oscillation surface, explains that the normalized overlap-defect weight section is already closed while the absolute neutrino spectrum still waits on the residual bridge scalar `B_nu`, with `lambda_nu` induced only after the exact `q_mean^p_nu` factorization is closed, and marks hadrons as execution-bound rather than paper-derived output rows.
