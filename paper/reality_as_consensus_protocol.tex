...
Theorem~\ref{thm:qbft-safety}). The repair map admits a quantum-channel formulation via the Petz
recovery map (Definition~\ref{def:oph-repair-map}), but the \textrm{CPTP} property on all inputs
requires either full-rank $\mathcal{N}(\sigma)$ or an explicit domain restriction; trace-preserving
completion is not automatic when $\mathcal{N}(\sigma)$ has a non-trivial kernel
