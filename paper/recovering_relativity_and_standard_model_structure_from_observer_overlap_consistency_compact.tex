...
The D10 calibration sector gives supplement-backed gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II calibration branch; the older freeze-once coherent repair surface is retained only as compare-only validation. The separate D6 cosmological-capacity relation remains an input-dependent corollary. The charged-lepton and Koide sectors remain phenomenological continuations rather than theorem-level spectra. What is exact there is narrower: the charged same-carrier readback law is closed, the charged absolute-scale underdetermination theorem is closed, and the theorem-grade waiting set is the charged sector-response / declaration functor \(\widehat C_e\) together with the common-shift-breaking affine anchor \(A_{\mathrm{ch}}\). On the flavor side, the structural output fixed before those continuations is the realized counting chain \(N_g=3\) then \(N_c=3\).
