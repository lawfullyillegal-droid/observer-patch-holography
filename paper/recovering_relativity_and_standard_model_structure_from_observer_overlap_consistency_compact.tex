...
The local MaxEnt branch gives a finite-range interacting fixed-cutoff dynamics, Propositions~\ref{prop:gauge} and \ref{prop:ancillauv} fix uniqueness of the UV branch only modulo OPH-stable equivalence, and Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} with Proposition~\ref{prop:hginteracting} close the genuinely noncentral topological branch at fixed cutoff. The open items on the UV side are the continuum BW/geometric lift of \textbf{T1}+\textbf{T3} and the fuller transportable compact-gauge lift when \textbf{T2} and \textbf{T5}--\textbf{T7} are invoked. The smallest honest internal extension route to the BW/geometric lift is the two-step chain of a scaling-limit cap-pair extraction theorem that extracts \((\mathcal A_\infty(C),\omega_\infty^C)\) as local weak-\(*\) limits of transported cap marginals with vanishing carried collar errors, followed by an ordered null cut-pair rigidity theorem that collapses the residual cap-preserving conformal freedom on those extracted cap pairs to the unique BW hyperbolic subgroup. Beneath the carried-collar witness, the sharper raw datum is a fixed-local-collar Markov/faithfulness package: collarwise conditional mutual information vanishing together with an eventual positive lower spectral bound on each fixed local collar model. That two-step chain and its fixed-local-collar datum are not part of the proved theorem surface stated here.

The D10 calibration sector gives supplement-backed gauge-coupling closure on the displayed carrier. That carrier closes its own exact \(W/Z\) chart, and the promoted target-free source-only repair theorem emits the public electroweak quintet on the Phase II calibration branch; the freeze-once coherent repair surface serves only as compare-only validation. The separate D6 cosmological-capacity relation remains an input-dependent corollary. The charged-lepton and Koide sectors remain phenomenological continuations rather than theorem-level spectra. What is exact there is narrower: the charged same-carrier readback law is closed, the charged absolute-scale underdetermination theorem is closed, and the theorem-grade waiting set is the charged sector-response / declaration functor \(\widehat C_e\), then the refinement-stable uncentered trace lift beneath the determinant-line route, and only then the common-shift-breaking affine anchor \(A_{\mathrm{ch}}\). A minimal operator-side extension route is a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; even if that route is internalized, the post-promotion uncentered trace lift remains a separate object. Its clean future realization is an uncentered charged response lift carrying a determinant line, with \(A_{\mathrm{ch}}=\tfrac13\log\det(Y_e)=\tfrac13\operatorname{tr}(\log Y_e)\). The smaller object beneath that determinant-line route is a refinement-stable uncentered trace lift. On the flavor side, the structural output fixed before those continuations is the realized counting chain \(N_g=3\) then \(N_c=3\).

The light-quark and hadron sectors remain weaker. A recovered-core nonzero light-quark pure-\(B\) selector is not derived, so the light-quark split remains a D12 continuation branch. On the CKM side the D12 sheet is a wrong-branch no-go, so the remaining quark branch object is one finite left-handed same-label relative-sheet orbit together with the provider/evaluator that exposes the first non-empty orbit elements and their discrete selector; a smaller local chirality-basis orbit is diagnostic-only because it leaves the ordered same-label left-eigenframe domain. The weighted-cycle neutrino continuation fixes the PMNS/hierarchy shape, closes the normalized same-label overlap-defect weight section beneath that shape, and leaves one positive absolute-scale scalar open through the residual bridge scalar \(B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}\) above the closed normalizer; the current attached stack factors through \(q_e = q_{\mathrm{mean}} qbar_e\), so a \(qbar_e\)-only bridge collapse does not follow from the present corpus. The best closed constructive local object beneath that scalar is the defect-weighted same-label edge family \(q_e=\sqrt{g_e d_e}\) together with its induced \(\mu_e\) family, but that family does not emit \(B_\nu\) by itself. No further discrete neutrino branch object appears on that lane. The hadron lane depends on nonperturbative runtime completion and stable-channel convergence rather than a closed theorem-level mass surface.
