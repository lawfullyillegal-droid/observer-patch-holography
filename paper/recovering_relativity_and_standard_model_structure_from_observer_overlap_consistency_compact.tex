...
The gravity chain still requires a bridge from modular data to a local stress tensor, but the sharp claim boundary is now four-layered. At fixed regulator scale one can transfer the same edge-center data used in the spatial collar theorem to regulated null strips. What does not follow from that transfer alone is the left/right tensor split of the strip multiplicity spaces needed for exact splice and modular-additivity identities, nor the half-line control needed to turn strip statements into genuine translation data. The null bridge therefore separates four layers:
\begin{enumerate}[leftmargin=2em]
\item transferred center data at the two null cuts;
\item an additional decomposition-inheritance hypothesis that upgrades those center sectors to the same left/right block structure used in Theorem~\ref{thm:markovcollar};
\item exact Markovity, or a controlled exact-Markov replacement on one fixed inherited strip model, yielding an exact-or-controlled four-term strip relation with an explicit carried defect operator;
\item endpoint-Lipschitz control for the renormalized half-line modular family, strong enough to define a weak tail generator before any Borchers promotion.
\end{enumerate}
Only after all four layers are in place does the null strip support the same HJPW factorization logic as the spatial collar branch. The later promotion of the weak tail generator to a genuine positive translation generator remains the separate half-sided modular inclusion premise \textbf{T6}, and the later identification of that generator with a continuum null stress tensor remains a separate scaling-limit premise under Assumption~\ref{ass:technical}\textbf{T8}.
...
\mathcal H_{i_+^{\alpha_+}}.
\]
Thus the two null cuts transfer the same theorematic center data as the spatial collar branch, while the left/right split of the strip multiplicity spaces is exactly the extra decomposition-inheritance hypothesis and is not forced by center transfer alone.
\end{proposition}

\begin{proof}
By complete reducibility of the finite-dimensional unitary actions \(\widehat K_\pm\), the displayed decompositions of the extended regulator Hilbert spaces exist. Because \(\mathcal A(J)\) is, by hypothesis, the algebra of endomorphisms commuting with the transported cut actions on \(\tilde{\mathcal H}_J\), every strip observable preserves the pair \((\alpha_-,\alpha_+)\) and acts only on the multiplicity space \(\mathcal H_{j^{\alpha_-,\alpha_+}}\). Therefore
\[
\mathcal A(J)
\cong
\bigoplus_{\alpha_-,\alpha_+}
...
For the glued tripartition, take invariants across each cut. Using the decompositions above,
\[
...
\mathcal H_{i_+^{\alpha_+}}.
\]
Taking \(\widehat K_-\times\widehat K_+\)-invariants and using Schur's lemma gives
\[
...
\mathcal H_{i_+^{\alpha_+}}.
\]
If item (iv) holds, substitute the factorization
\(\mathcal H_{j^{\alpha_-,\alpha_+}}
\cong
\mathcal H_{j_L^{\alpha_-,\alpha_+}}\otimes
\mathcal H_{j_R^{\alpha_-,\alpha_+}}\)
to obtain the displayed inherited split and the stated action of the left and right union algebras. Nothing in the Schur-lemma argument itself forces item (iv); it is the additional structural condition required to reproduce the same blockwise tensor pattern as Theorem~\ref{thm:markovcollar}.
\end{proof}
...
\right\}.
\]
If the reference state \(\omega\) is exact Markov on this inherited decomposition, then there exists a central operator
\[
...
\]
and the carried defect operator
\[
...
\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon).
\]
If the marginals entering \(\Delta K_J\) are uniformly faithful with lower spectral bound \(\lambda_\ast>0\), then
\[
...
\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon).
\]
Independently, the Fawzi--Renner recovery map supplies a comparison state with trace-norm error
\[
r_{\mathrm{FR}}(\varepsilon)
=
2\sqrt{1-e^{-\varepsilon}}
...
\begin{proof}
On the inherited split of Proposition~\ref{prop:nullec}, exact Markovity is equivalent, by the HJPW structure theorem, to a blockwise factorization
\[
\omega_{I_-JI_+}
=
\bigoplus_{\alpha_-,\alpha_+}
p_{\alpha_-,\alpha_+}\,
\omega_{I_-\,j_L^{\alpha_-,\alpha_+}}^{(\alpha_-,\alpha_+)}
\otimes
\omega_{j_R^{\alpha_-,\alpha_+}\,I_+}^{(\alpha_-,\alpha_+)},
\]
and the modular Hamiltonian is additive on each block. The only non-additive remainder is therefore the direct sum of block constants, which is central and gives \(K_{\partial,J}(\omega)\).

For the controlled statement, the same compactness argument as Proposition~\ref{prop:controlledmarkov} applies verbatim on this fixed finite-dimensional strip model, with \(A,B,D\) relabeled as \(I_-,J,I_+\). The observable bound is immediate from the trace-norm estimate. If the relevant marginals are bounded below by \(\lambda_\ast\), the operator-logarithm Lipschitz estimate used earlier in Proposition~\ref{prop:approxrecover} yields
\[
\|\mathfrak D_J(\omega,\widetilde\omega_J)\|_\infty
\le
4\lambda_\ast^{-1}\,
\|\omega-\widetilde\omega_J\|_1,
\]
which gives the displayed control. The Fawzi--Renner bound provides a constructive recovered comparison state but does not by itself identify an exact Markov state; that exact replacement is measured by the modulus \(\delta^{\mathrm M}_{I_-:J:I_+}\).
\end{proof}

\begin{definition}[Renormalized null modular functional]
For a null interval \(I\) on generator \(\Omega\), write
\[
\widetilde K[I,\Omega]
:=
K[I,\Omega]-K_\partial(I,\Omega)
\]
for the full modular Hamiltonian with the central endpoint-label term removed. For a null half-line \(H_a:=(a,\infty)\), abbreviate
\[
...
\begin{proof}[Proof sketch]
The bounded-interval estimate already derived from Axiom~\ref{ax:maxent} gives
\[
\bigl|\langle\psi,(K[I']-K[I])\phi\rangle\bigr|
\le
C_{\psi,\phi}\,|I'\Delta I|.
\]
For bounded intervals \(I=(a,b)\) and \(I'=(a',b')\),
\[
|I'\Delta I|
\le
|a'-a|+|b'-b|,
\]
which yields the displayed joint endpoint-Lipschitz bound after removal of the central endpoint term. For half-lines \(H_a=(a,\infty)\) and \(H_{a'}=(a',\infty)\), removal of the endpoint term leaves a difference supported on the bounded interval between the two endpoints, so
\[
|H_{a'}\Delta H_a|=|a'-a|,
\]
and the same argument gives the half-line estimate. Lipschitz functions on compact intervals are absolutely continuous, hence possess distributional derivatives in \(L^\infty_{\mathrm{loc}}\). The final integral identity is the fundamental theorem of calculus applied to the matrix-element function \(f_{\psi,\phi}(a)\).
\end{proof}

\begin{theorem}[Half-line density upgrade and second-derivative formula]\label{lem:density}
...
\]
Thus the weak tail generator of Proposition~\ref{prop:n3} admits a density upgrade precisely under the additional weak differentiability and tail-vanishing conditions.
\end{theorem}
...
-\frac{1}{2\pi}\,\partial_a f_{\psi,\phi}(a).
\]
The additional weak differentiability assumption defines \(p(a,\Omega)\) distributionally by
\[
\langle\psi,p(a,\Omega)\phi\rangle
:=
-\partial_a q_{\psi,\phi}(a).
\]
Integrating this identity from \(a\) to \(+\infty\) and using \(q_{\psi,\phi}(a)\to 0\) gives
\[
q_{\psi,\phi}(a)
=
\int_a^\infty \langle\psi,p(v,\Omega)\phi\rangle\,dv.
\]
Integrating once more and using \(f_{\psi,\phi}(a)\to0\) yields
\[
f_{\psi,\phi}(a)
=
2\pi\int_a^\infty (v-a)\,\langle\psi,p(v,\Omega)\phi\rangle\,dv,
\]
which is the required half-line formula. The second-derivative identity is merely the definition of \(p\) rewritten in terms of \(f_{\psi,\phi}\).
\end{proof}

\begin{corollary}[Geometric form of the assumed half-sided inclusion]\label{cor:n2}
Assume in addition the standard half-sided modular inclusion premise \textbf{T6} for the null half-line blow-up algebras,
\[
\sigma_t^\omega(\mathcal A(v>v_1))
\subseteq
\mathcal A(v>v_1)
\qquad
(t\ge 0).
\]
Then the blow-up limit of Theorem~\ref{thm:bw} near a smooth entangling cut acts geometrically by the null dilation
\[
v-v_0 \mapsto e^{-2\pi t}(v-v_0),
\]
which is the geometric realization of the assumed half-sided inclusion on null half-lines. This step does not prove \textbf{T6} from fixed-cutoff strip data; it identifies the geometric form that \textbf{T6} takes on the OPH geometric branch.
\end{corollary}

\begin{proof}
Near a smooth cut, the cap dilation from Theorem~\ref{thm:bw} becomes a Rindler boost in the tangent geometry. Restricting that boost to a null generator gives the stated dilation. For \(t\ge 0\), the half-line \(v>v_0\) maps into itself, which is exactly the geometric form of the assumed half-sided inclusion.
\end{proof}

\begin{lemma}[Borchers promotion to a genuine null translation generator]\label{lem:translation}
Under the half-sided modular inclusion premise of Corollary~\ref{cor:n2}, the weak tail generator of Proposition~\ref{prop:n3} is promoted to a genuine one-parameter unitary group \(U(a)=e^{iaP}\) with \(P\ge 0\) such that
\[
...
\begin{proof}[Proof sketch]
This is the Borchers--Wiesbrock consequence of half-sided modular inclusion. The modular group of the larger algebra acts by endomorphisms on the smaller one for one sign of modular time, and that structure produces a positive translation generator \(P\) whose affine scaling law is the one written above. The point of Proposition~\ref{prop:n3} is only to define the weak tail derivative prior to this promotion; positivity and unitary implementation first appear here, under \textbf{T6}.
\end{proof}

\begin{theorem}[Conditional null-stress identification from modular data]\label{thm:stress}
Assume Axioms~\ref{ax:screen}--\ref{ax:entropy}, Assumption~\ref{ass:technical}, Proposition~\ref{prop:nullec}, the exact-or-controlled strip hypothesis of Corollary~\ref{cor:n1}, Proposition~\ref{prop:n3}, the density-upgrade hypotheses of Theorem~\ref{lem:density}, the half-sided inclusion premise \textbf{T6}, and the scaling-limit null-stress identification premise of the relativistic EFT branch. Then there exists an operator-valued distribution \(T_{kk}(v,\Omega)\) such that the renormalized modular Hamiltonian on a null half-line takes the form
\[
...
\begin{proof}[Proof sketch]
Proposition~\ref{prop:nullec} and Corollary~\ref{cor:n1} supply the precise null-strip statement of the collar factorization package: transferred cut centers, an explicit decomposition-inheritance condition, and exact or controlled four-term modular additivity on one fixed inherited strip model, with the finite-stage failure carried by the defect operator \(\mathfrak D_J\). Proposition~\ref{prop:n3} defines the weak tail generator of the renormalized half-line family from endpoint-Lipschitz control. Theorem~\ref{lem:density} upgrades that weak generator to a density under the additional differentiability and tail assumptions. Corollary~\ref{cor:n2} and Lemma~\ref{lem:translation} then promote the weak tail generator to the genuine positive Borchers translation generator under the separate half-sided inclusion premise \textbf{T6}. In the relativistic scaling-limit branch, the null-stress identification premise upgrades the resulting density to \(T_{kk}\). The three carried remainder sources are the constructive Fawzi--Renner comparison error, the exact-Markov replacement error on the fixed strip model, and the separate EFT/scaling-limit remainder \(\eta_I^{\mathrm{EFT}}\). If bounded null intervals inherit geometric modular action, the same density transports to finite intervals with an interval-dependent affine kernel \(g_I(v)\). The ambiguity
\[
...
The fixed-cutoff advance made in this subsection is the rigorous transfer of cut-center data to null strips together with the identification of the exact extra hypothesis needed to inherit the collar-type left/right split. Exact N1 then holds only for exact Markov strip states on that inherited decomposition, or along controlled families with \(\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon_J)\to 0\) on one fixed strip model. N2 remains the separate half-sided inclusion premise \textbf{T6}, and the final identification of the half-line density with a local continuum \(T_{kk}\) remains a scaling-limit EFT premise under \textbf{T8}. Thus the null bridge now uses the same theorematic center decomposition as the spatial collar branch whenever center transfer and decomposition inheritance hold, but it does not claim a fixed-cutoff proof of half-sided inclusion or of a local continuum stress tensor.
\end{remark}
