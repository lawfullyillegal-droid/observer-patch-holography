...
\paragraph{Current-premise no-go.}
Let
\[
...
\]
Then the present corpus does not emit the three closure objects
\[
\Theta_{ud}^{\mathrm{mass}},
\qquad
\Theta_{ud}^{\mathrm{phys}},
...
\Theta_{ud}^{\mathrm{abs}}.
\]
The reason is exact and split by layer. On the mass side, \(P\) emits the one-parameter ray \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\) but not a theorem-grade nonzero value of its modulus \(t_1\). On the mixing side, \(P\) emits the negative selector closure \(\sigma_{ud}=\sigma_{\mathrm{ref}}\), and same-sheet rephasing preserves CKM moduli, so \(P\) emits no sector-attached same-label left-handed lift from \(\sigma_{\mathrm{ref}}\) to the physical CKM shell. On the absolute side, the affine sector-mean split is formula-complete on the selected sheet, but the exact sextet witness remains target-anchored and \texttt{current\_family\_only}, so \(P\) emits no target-free physical-sheet pair \((g_u,g_d)\). Hence the exact same-family sextet and the D12 internal backread scalar package are sidecar representatives on their declared surfaces, not \(P\)-emitted theorem outputs.
