...
\textbf{D7} & Compact gauge reconstruction (Theorem~\ref{thm:compactgauge}) & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T6 together with T7's directed-colimit/fiber premise for transportable edge sectors; the local-Gibbs/collar-mixing package supplies only fixed-cutoff support and recoverability control and does not by itself determine whether that colimit is nontrivial & Phase I conditional structural theorem \\
...
This local-Gibbs/Lieb--Robinson package should not be read as a proof that the refinement limit lands in a trivial gauge phase, as a proof that transportable edge sectors persist nontrivially under refinement, or as a proof that OPH has selected a unique microscopic realization. Its theorem-level role is collar-local mixing together with the carried support/error control used by the later fixed-cutoff and scaling-limit arguments.
...
\item[\textbf{T7}] whenever compact gauge reconstruction is invoked, the sector category means the directed colimit of transportable edge sectors with objectwise finite-dimensional fibers; the existence, transportability, and possible nontriviality of this colimit are part of the categorical premise rather than a consequence of Axiom~\ref{ax:maxent} or of the fixed-cutoff collar-mixing package.
\end{itemize}
\end{assumption}
...
\textbf{D7} & Theorem~\ref{thm:compactgauge}: compact gauge reconstruction & Axioms~\ref{ax:screen}--\ref{ax:entropy} & Doplicher--Roberts / Tannaka reconstruction & T2 and T5--T6 together with T7's directed-colimit/fiber premise for transportable edge sectors; the local-Gibbs/collar-mixing package supplies only fixed-cutoff support and recoverability control and does not by itself determine whether that colimit is nontrivial & conditional structural theorem \\
...
\begin{proof}
Items (i)--(iii) are collected from Theorem~\ref{thm:confluence}, Corollary~\ref{cor:lorentz}, Theorem~\ref{thm:einstein}, and Corollary~\ref{cor:einstein}. The compact gauge reconstruction is Theorem~\ref{thm:compactgauge}, applied under the explicit transportable sector-colimit premise of \textbf{T7}; the preceding MaxEnt/mixing package is not used there to prove nontriviality of that colimit. The product gauge structure up to finite quotient is Theorem~\ref{thm:sm}; and the final identification of the exact Standard Model quotient is Theorem~\ref{thm:hypercharge} together with Corollaries~\ref{cor:ng}, \ref{cor:nc}, and Proposition~\ref{prop:z6}.
\end{proof}
...
\]
after transport to each fixed local collar model. Only after that witness is supplied can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Beneath that derived schedule, the sharper raw datum is the transported fixed-local-collar Markov/faithfulness package.
...
At any fixed UV cutoff, edge-center completion equips collars with finitely many sector labels and fusion rules. The local MaxEnt / collar-mixing package established earlier controls fixed-cutoff recoverability, modular-support localization, and carried error terms on the realized branch. It does not by itself prove that transportable edge sectors persist as nontrivial objects under refinement, and it does not by itself determine whether the refinement limit lies in a trivial or nontrivial gauge phase. Whenever compact gauge reconstruction is invoked, that transportable refinement-directed sector structure is the explicit content of \textbf{T7}.
...
\]
for the directed colimit retaining the transportable sectors and intertwiners that persist in that system, with objectwise finite-dimensional fibers. This is the category on which Doplicher--Roberts / Tannaka reconstruction is applied.
...
\]
together with the already-realized discrete gauge data of the structural branch, in particular
\[
N_c=3,
\qquad
...
\]
The factor \(e^{-2\pi}\) is the same modular normalization already used on the Lorentz/BW side, and \(P^{1/6}\) is the cell-area scaling relation carried by the present D10 implementation.
...
The pixel area \(P\) calibrates the separate gauge-coupling sector used here, but the circular step that read the internal D10 transmutation parameter back from measured low-energy couplings is no longer part of the compact-paper presentation. Within the printed D10 implementation, \(P\) first fixes \(M_U(P)\) and \(E_{\mathrm{cell}}(P)\), then a one-dimensional pixel-closure solve fixes \(\alpha_U(P)\), and only then do \(t_2(P)\), \(t_3(P)\), \(v(P)\), and the running electroweak outputs appear. Quantities algebraically entangled with that calibration are therefore compare-only consistency checks for that implementation rather than part of the recovered-core claim set.
...
\]
The intrinsic neutrino lane factors exactly through this certificate. The quark mass-side continuation compresses transported same-label data to
\[
(\Delta_{ud}^{\mathrm{overlap}},\eta_Q^{\mathrm{centered}})
\]
and, on the compact same-family branch, to the emitted ray \(\mathcal R_{D12}^{ud}\). The selector-side frontier is no longer an open missing object: the local same-label left-handed orbit already closes to the singleton \(\sigma_{\mathrm{ref}}\), so the selected branch is fixed but remains the current wrong-branch D12 sheet. The next exact theorem-side quark object is therefore the intrinsic scale law on the emitted D12 mass ray, not another selector theorem or a fresh continuous Yukawa parameter. Any texture exponent or defect count quoted on D12 surfaces is at most a compare-only summary of lane-specific excitation action in units of \(\log 6\), not an additional recovered-core flavor datum.

\paragraph{Charged-lepton continuation status.}
The charged-lepton and Koide sectors remain phenomenological continuations rather than theorem-level spectra. What is exact there is narrower: the charged same-carrier readback law expresses the centered charged logs once a charged source pair is emitted, and the charged absolute-scale underdetermination theorem closes the common-shift no-go. The theorem-grade waiting set is the promotion
\[
\widehat C_e^{\mathrm{cand}}\longrightarrow \widehat C_e,
\]
and the post-promotion frontier is now smaller than the older matrix-language summary suggested. Once \(\widehat C_e\) exists and the lift is refinement-stable on theorem-grade physical \(Y_e\), the family cocycle descends to one physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), with
\[
...
\]
A minimal operator-side extension route is still a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; internalizing that route can promote \(\widehat C_e^{\mathrm{cand}}\), but it does not by itself emit the descended physical scalar. Its clean future realization remains an uncentered charged response lift carrying a determinant line, with
\[
A_{\mathrm{ch}}=\tfrac13\log\det(Y_e)=\tfrac13\operatorname{tr}(\log Y_e).
\]

\paragraph{Quark continuation status.}
The light-quark sector remains weaker. A recovered-core nonzero light-quark pure-\(B\) selector is not derived, so the light-quark split remains a D12 continuation branch. On the CKM side the current D12 sheet is a wrong-branch no-go, but the local selector frontier is now closed rather than merely narrowed: the emitted same-label left-handed orbit collapses to the singleton \(\sigma_{\mathrm{ref}}\). This is an honest negative closure, not a repaired physical branch: \(\sigma_{\mathrm{ref}}\) is still the current D12 reference sheet, so the physical CKM-shell mismatch survives unchanged on the selected branch. A smaller local chirality-basis orbit remains diagnostic-only because it leaves the ordered same-label left-eigenframe domain. The mass side is now theorem-sharp as well: an explicit D12 underdetermination theorem shows that the current same-family branch emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\) rather than a distinguished modulus on that ray. The next exact object on the emitted D12 branch is therefore not another sheet selector but the intrinsic scale law on the already-emitted mass ray \(D12_{ud}^{\mathrm{mass}}\).
...
\]
This weighted-cycle branch reaches the PMNS window and the correct splitting hierarchy on the current continuation surface, and no hidden extra discrete branch is left on that lane.

What it does \emph{not} yet do is emit the absolute normalization as a theorem. The normalized same-label overlap-defect weight section is already closed beneath the weighted-cycle shape, and the paper-facing remaining freedom is one positive absolute-scale bridge scalar
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
...
\]
The current attached stack factors through \(q_e=q_{\mathrm{mean}}qbar_e\), so a \(qbar_e\)-only bridge collapse does not follow from the present corpus. The older selected-point scalar built from the weighted-cycle state is already internal to the emitted \(qbar_e\), \(\psi_{\mathrm{wc}}\), and \(\psi_\ast\) data and therefore is not the missing bridge-external object. Above the best emitted proxy, the exact missing bridge object is smaller: the near-unity positive correction
\[
C_\nu:=\frac{B_\nu}{I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}},
\]
whose live compare-only value is \(C_\nu^\ast=0.9995473560\). Because the proxy is already internal to the emitted stack, this reduced correction invariant \(C_\nu\) is now the irreducible exact missing object, while the paper-facing amplitude \(B_\nu\) is reconstructed above it. The direct correction audit gives the target-containing reduced window \(C_\nu\in[0.9994296,1.0015784]\), which induces the sharpened compare-only bridge window \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\), improving on the older three-route corridor. A narrower shortlist-consensus window \([6.70484,6.71259]\) is also on disk, but it stays diagnostic-only because it does not contain the live compare-only target.
...
\]
but that family does not emit \(C_\nu\) or the induced \(B_\nu\) by itself. The sharpest current local interface for the missing bridge remains the Majorana overlap-defect scalar route, yet the actual open object above the normalizer is now the reduced correction invariant \(C_\nu\), not another hidden branch selector. The strongest closed-form residual clue remains compare-only and must stay that way until derived.
