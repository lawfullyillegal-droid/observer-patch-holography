...
The collar analysis identifies where modular nonadditivity can live, but by itself it does not determine the continuum geometric action of the limiting modular group. The correct theorem-scope is therefore branch-conditional and must be stated explicitly. What OPH supplies here is a controlled reduction from the regulator collar to the refinement limit, together with the \(2\pi\) normalization and carried-error statement \emph{once} the refinement-stable cap net lies in the Bisognano--Wichmann geometric modular class of \textbf{T4}. No selector-free uniqueness claim is made inside the circle stabilizer: cap-preservation alone does not determine a unique noncompact subgroup of \(\Conf^+(S^2)\).
...
The theorem proves the controlled collar reduction together with the \(2\pi\) normalization \emph{on the OPH refinement branch in which the cap net has the Bisognano--Wichmann geometric modular action of \textbf{T4}}. It does not claim that cap-preservation alone singles out a unique noncompact subgroup inside \(\Conf^+(S^2)\); that identification is part of the branch content of \textbf{T4}, not a separate selector-free group-theoretic consequence. The attached source set does not prove that the bare OPH axioms alone force the existence of that branch. The remaining nontrivial burden is therefore phase selection in the refinement limit, not the collar-local reduction or the normalization once the geometric branch is realized.
\end{remark}
