...
and \(N_g = 3\) (see §6.2 and GAUGE\_GROUP\_DERIVATION.md).
...
- N2 + density \ensuremath{\Longrightarrow} T\_kk \ensuremath{\Longrightarrow} Einstein (Thm 5.1)
...
- N1-N3 (derived, §5.2) \ensuremath{\Longrightarrow} local K{[}I{]} and P = \ensuremath{\int} T\_kk dv
...
See GAUGE\_GROUP\_DERIVATION.md for the complete formal statement, admissibility
...
$\Phi_{\ensuremath{\ell}\ensuremath{\to}L}$ around the MaxEnt state \ensuremath{\omega}\_\ensuremath{\ell}. Constraint-
preserving perturbations \ensuremath{\delta}\ensuremath{\rho} evolve as
\ensuremath{\delta}\ensuremath{\rho}' = $\Phi_{\ensuremath{\ell}\ensuremath{\to}L}$(\ensuremath{\delta}\ensuremath{\rho}). A relevant operator corresponds
...
\textbf{Proposition 6.9 (The number of generations is N\_g = 3).} Under (i)
...
stability selecting minimal viable spectrum, and (iv) the derived N\_c = 3
...
4. Use the already-derived N\_c = 3 from Theorem 6.14.

\textbf{Step 1: CP violation lower bound.} The number of physical CP-violating
phases in an N\_g \ensuremath{\times} N\_g CKM matrix is:
...
\textbf{Proof.} Conjugation by U\_ki sends $\pi_i$(O) to $\pi_k$(O), by U\_jk to $\pi_j$(O),
by U\_ij back to $\pi_i$(O). Thus conjugation by $\Omega_{ijk}$ fixes $\pi_i$(O), so
$\Omega_{ijk}$ commutes with $\pi_i$(O). QED.

\textbf{Lemma 6.11 (gauge behavior).} If \~{U}\_ij = V\_i U\_ij V\_j\ensuremath{\dagger} with V\_i
...
In particular, if $\Omega_{ijk}$ is central, its class is gauge invariant. QED.
...
$\varphi_{ij}$ $\varphi_{jk}$ $\varphi_{kl}$ $\varphi_{li}$ on a quadruple overlap to
...
group SU(N\_c) \ensuremath{\times} SU(2) \ensuremath{\times} U(1)\_Y and one generation of left-handed Weyl
...
With these relations, SU(N\_c)\textsuperscript{2}U(1) and U(1)\textsuperscript{3} anomalies vanish
...
Without Yukawas, the cubic anomaly leaves two discrete branches (Y\_u, Y\_d
exchange). Yukawa invariance selects the branch with a single Higgs doublet.
QED.

\textbf{Corollary 6.13a (Exact rational hypercharges).} With the derived N\_c = 3,
...
Under the gauge structure SU(N\_c) \ensuremath{\times} SU(2)\_L \ensuremath{\times} U(1)\_Y with one
...
N\_c to be odd. Under MAR (lexicographic minimization of \(C(\Sigma)\),
...
\textbf{Inputs.}
1. Low-energy gauge group contains an SU(2)\_L factor and an SU(N\_c)
color factor.
...
The Witten constraint alone allows N\_c \ensuremath{\in} \{1, 3, 5, 7, \ldots\}. N\_c = 1
fails admissibility (SU(1) is trivial, no complex nonabelian charge type).
MAR (input 4) then selects N\_c = 3 as the smallest nontrivial value. QED.

\textbf{Status.} The Witten anomaly derives N\_c odd. The specific value N\_c = 3
...
\item
  ...
  breaking.
\item
  A gauge boson is the quantum of the gauge field. Once U(1)\_em emerges from
...
\tightlist
\item
  Theorem 4.2 (BW\_\{S\textsuperscript{2}\}) shows that under collar Markov locality, MaxEnt
...
\item
  ...
  \ensuremath{\delta}S\_gen = 0 implies the semiclassical Einstein equations in the EFT regime.
\item
...
\textbf{Experimental status}: PDG 2025 lists m\_g \ensuremath{\leq} 1.76 \ensuremath{\times} 10\textsuperscript{-23}
eV/c\textsuperscript{2} (90\% CL) from gravitational wave dispersion analysis. The GW speed
bound from GW170817 constrains (c\_GW \ensuremath{-} c)/c to \textasciitilde10\textsuperscript{-15}. The
...
The edge-center completion (Theorem 2.3) yields sector probabilities p\_\ensuremath{\alpha}
...
\textbf{Abelian case (Z\_n).} For a Z\_n gauge theory, the edge
...
and the probabilities are p\_\{v,q\} = \ensuremath{\langle}P\_\{v,q\}\ensuremath{\rangle}. This
...
$\mathcal{H}_{edge}^{phys}$ = $\oplus_R$ W\_R where W\_R
contains states with flux in representation R.

\emph{Step 3 (Natural Hamiltonian).} From LG, the MaxEnt generator restricted to
edge modes takes the form H\_\{edge\} = $\Sigma_R$ h\_R P\_R where P\_R is
the projector onto the R-sector. The key claim is that h\_R = $\lambda_R$.

\emph{Justification:} The group Laplacian $\Delta_G$ = \ensuremath{-}$\Sigma_a$ (T\^{}a)\textsuperscript{2} is the
...
same role: $\lambda_R$ = \textbar S\textbar{} \ensuremath{-} (1/d\_R) $\Sigma_{s\ensuremath{\in}S}$ $\chi_R$(s).
...
p\_R = Tr($\rho_{edge}$ P\_R). The effective dimension for
entanglement is d\_R (not d\_R\textsuperscript{2}) because we trace over one side of the
cut. This gives:
...
\textbf{Why d\_R and not d\_R\textsuperscript{2}?} The full edge space has dimension d\_R\textsuperscript{2} in
sector R (from V\_R \ensuremath{\otimes} V\_R\emph{), but entanglement entropy measures
correlations }across* the cut. After tracing over one side, the reduced density
matrix has effective rank d\_R. Mathematically: in the Markov normal form, the
edge factor on one side contributes log d\_R to the entropy.
...
weight(R) \ensuremath{\propto} exp(\ensuremath{-}A E\_R) = exp(\ensuremath{-}g\textsuperscript{2} A C\textsubscript{2}(R)/2). Comparing with the
heat-kernel expansion K\_t(U) = $\Sigma_R$ d\_R $\chi_R$(U) e\^{}\{\ensuremath{-}t C\textsubscript{2}(R)\} yields the
...
reduces to an effective 2D YM with known ``Euclidean thickness'' A\_eff:
...
explicit 2D Z\_n gauge models on closed geometries.

\textbf{Model.} A 2\ensuremath{\times}2 periodic lattice gauge theory (8 links) with
Z\_n link Hilbert spaces and Hamiltonian:

\[
H = -K \sum_p \mathrm{Re}(B_p) - h \sum_\ell \mathrm{Re}(X_\ell) - \Gamma \sum_v \mathrm{Re}(A_v),
\]

where X\_\ensuremath{\ell} is the Z\_n shift on link \ensuremath{\ell}, B\_p is the oriented
plaquette operator (product of Z's around plaquette p), and A\_v is the
oriented star/Gauss operator (outgoing X, incoming X\ensuremath{\dagger}). With
K = 1 and \ensuremath{\Gamma} = 5, the ground state satisfies \ensuremath{\langle}A\_v\ensuremath{\rangle} = 1
...
edge charge is the restricted star Q\_v\^{}\{(A)\} = $\prod_{\ensuremath{\ell} \ensuremath{\in} star(v) \ensuremath{\cap} A}$ X\_$\ell^{\ensuremath{\pm}1}$.
...
The equality t\_\{q=1\} = t\_\{q=2\} is the crucial \textbf{overconstrained} check: at
h = 1.0, extracting t from q = 1 and q = 2 independently gives
t\_\{q=1\} \ensuremath{\approx} 1.2956389318579 and t\_\{q=2\} \ensuremath{\approx} 1.2956389318521. The
...
\textbf{Region-choice robustness.} At h = 1, the extracted g\_ent
is nearly independent of region size:
- 2 links (one vertex's outgoing links): g\_ent \ensuremath{\approx} 0.453
...
where d\_R is the dimension of irrep R, $\chi_R$ is its character, and
Q\_v\^{}\{(A)\}(h) is the restricted gauge action at boundary vertex v acting
...
by the character states \{\textbar $\chi_R$\ensuremath{\rangle}\}. In this basis, the edge-sector
probabilities are exactly p\_R = \textbar c\_R\textbar\textsuperscript{2} where \textbar \ensuremath{\psi}\textsubscript{0}\ensuremath{\rangle} = $\Sigma_R$ c\_R
\textbar $\chi_R$\ensuremath{\rangle}. This is not an approximation; it is an exact identity for the
one-plaquette gauge-invariant sector.

The heat-kernel ansatz predicts p\_R \ensuremath{\propto} d\_R exp(\ensuremath{-}t $\lambda_R$). Extracting
t independently from the sign and standard irreps provides an overconstrained
test: the ratio $\lambda_{sign}$/$\lambda_{std}$ = 6/3 = 2 is a
...
The ``\ensuremath{\Delta}t/t'' column shows the fractional difference (t\_sign \ensuremath{-}
t\_std) / t. The ``log-ratio'' column shows
log(p\_sign/p\textsubscript{0}) / log(p\_std/(2 p\textsubscript{0})), which should
equal $\lambda_{sign}$/$\lambda_{std}$ = 2 if the heat-kernel
form holds exactly.
...
(finite truncated irrep basis, n\_max = 12, \ensuremath{\kappa} = 2) has been used to
...
suggests fitting A\_eff(g\textsuperscript{2}) = A\textsubscript{0} + a \ensuremath{\cdot} g\textsuperscript{2}. A weighted linear
fit gives:
...
converges to A\_eff \ensuremath{\to} 2 as g\textsuperscript{2} \ensuremath{\to} 0.
...
absolute coupling predictions: once A\_eff is determined from
microphysics, the conversion g\textsuperscript{2} = 2t/A\_eff fixes the gauge
...
\item
  \textbf{S\textsubscript{3}}: Casimir log-ratio ($\lambda_{sign}$/$\lambda_{std}$ = 2), precision 0.01\%
\item
...
O = $\Sigma_p$ Re(B\_p), the extracted masses m\_plaq are
...
scheme. The ratio m\_plaq / g\_ent\textsuperscript{2} is a pure number
...
where $\Lambda_{QCD}$ is the dimensional transmutation scale and F is a
...
\item
  ...
  MS-bar, then RG-run to predict $\alpha_s$(M\_Z), sin\textsuperscript{2}$\theta_W$(M\_Z).
\item
  ...
  $\Lambda_{QCD}$ as the first real mass-scale prediction.
\end{enumerate}
...
$\alpha_s$ consistency is a \emph{consistency check} with known physics, not a
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Canonical GUT normalization}: \ensuremath{\alpha}\textsubscript{1} \ensuremath{\equiv} (5/3)$\alpha_Y$, the
...
\item
  ...
  \tightlist
  \item
    \ensuremath{\alpha}\textsuperscript{(5)}(M\_Z\textsuperscript{2})\textsuperscript{-1} = 127.930 \ensuremath{\pm} 0.008 (MS-bar)
  \item
    \^{s}\textsuperscript{2}\_Z \ensuremath{\equiv} sin\textsuperscript{2}$\theta_W$(M\_Z\textsuperscript{2}) = 0.23122 \ensuremath{\pm} 0.00006 (MS-bar)
  \item
    $\alpha_s$(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009 (from EW fit; world average 0.1180)
  \end{itemize}

  \emph{Note on sin\textsuperscript{2}$\theta_W$ schemes:} PDG lists multiple definitions with different values.
...
\textbf{Derived couplings at M\_Z.} From $\alpha_{em}$ and sin\textsuperscript{2}$\theta_W$:
...
- A\textsubscript{1} \ensuremath{\equiv} \ensuremath{\alpha}\textsubscript{1}\textsuperscript{-1}(M\_Z) \ensuremath{\approx} 59.00
- A\textsubscript{2} \ensuremath{\equiv} \ensuremath{\alpha}\textsubscript{2}\textsuperscript{-1}(M\_Z) \ensuremath{\approx} 29.59
- A\textsubscript{3} \ensuremath{\equiv} $\alpha_s$\textsuperscript{-1}(M\_Z) \ensuremath{\approx} 8.47

\textbf{Analytic prediction formula.} Define A\_i = $\alpha_i$\textsuperscript{-1}(M\_Z) and
L = ln(M\_U/M\_Z). The RG equations give A\_i = A\_U + (b\_i/2\ensuremath{\pi})L. Taking
...
Once beta coefficients are fixed, $\alpha_s$(M\_Z) is completely determined by
...
with M\_U \ensuremath{\approx} 1.0 \ensuremath{\times} 10\textsuperscript{13} GeV and $\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 42.4.

\textbf{Comparison to measurement}: The PDG 2025 EW-fit value is
$\alpha_s$(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009. The SM-only prediction misses by
\ensuremath{\Delta}$\alpha_s$ \ensuremath{\approx} 0.047, a \textasciitilde52\ensuremath{\sigma} discrepancy, far too large to be rescued
...
with M\_U \ensuremath{\approx} 2.0 \ensuremath{\times} 10\textsuperscript{16} GeV and $\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 24.34 \ensuremath{\pm} 0.01.

\textbf{Comparison to measurement}: The PDG 2025 EW-fit value is
$\alpha_s$(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009. The mismatch is
...
\textbf{Significance}: SM-only unification predicts $\alpha_s$(M\_Z) \ensuremath{\approx} 0.071,
...
are active all the way down to M\_Z. If the \ensuremath{\Delta}b only turns on above
...
The predicted $\alpha_s$(M\_Z) depends sensitively on M\_*:

\begin{itemize}
\tightlist
\item
  \textbf{M\_* = M\_Z}: $\alpha_s$(M\_Z) = 0.1166, M\_U = 2.0\ensuremath{\times}10\textsuperscript{16} GeV
\item
  \textbf{M\_* = 1 TeV}: $\alpha_s$(M\_Z) = 0.1100, M\_U = 9.6\ensuremath{\times}10\textsuperscript{15} GeV
\item
  \textbf{M\_* = 10 TeV}: $\alpha_s$(M\_Z) = 0.1043, M\_U = 4.9\ensuremath{\times}10\textsuperscript{15} GeV
\end{itemize}
...
$\alpha_s$\textsuperscript{-1} by \textasciitilde0.6 to reach the experimental value.

\textbf{Inverted problem: derive M\_S from measured couplings.} With three measured
couplings (A\textsubscript{1}, A\textsubscript{2}, A\textsubscript{3}) and three unknowns (M\_S, M\_U, $\alpha_U$), the
...
\item
  \textbf{M\_U \ensuremath{\approx} 2.27 \ensuremath{\times} 10\textsuperscript{16} GeV}
\item
  \textbf{$\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 24.0}
\end{itemize}
\end{quote}

The uncertainty is dominated by the experimental error on $\alpha_s$(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009.
The central value is sensitive to the precise $\alpha_s$ input: $\alpha_s$ = 0.1175 gives
M\_S \ensuremath{\approx} 67 GeV, while $\alpha_s$ = 0.1166 (the MSSM prediction) gives M\_S \ensuremath{\approx} 91 GeV.
...
above M\_Z (say, at multi-TeV), then the gap must be filled by one of:
...
\textbf{Experimental status.} Minimal SU(5) GUTs predict $\tau_p$ \textasciitilde{} 10\textsuperscript{31}
years; Super-Kamiokande has pushed limits to $\tau_p$ \textgreater{} 10\textsuperscript{34} years,
...
unification} (MSSM-like $\alpha_s$ consistency) with \textbf{proton stability}
...
\ensuremath{\to} one-loop RG \ensuremath{\to} consistency check for $\alpha_s$(M\_Z) \ensuremath{\to} spectrum
...
- Relative precision of a\_cell: \ensuremath{\delta}a\_cell/a\_cell \ensuremath{\approx} 2.2 \ensuremath{\times} 10\textsuperscript{-5}
- Relative precision of \ensuremath{\sqrt{}}a\_cell: \ensuremath{\approx} 1.1 \ensuremath{\times} 10\textsuperscript{-5}

With the currently derived value a\_cell/$\ell_p$\textsuperscript{2} \ensuremath{\approx} 1.63094, this gives a\_cell \ensuremath{\approx} 4.26 \ensuremath{\times} 10\textsuperscript{-70} m\textsuperscript{2}, with irreducible CODATA uncertainty \ensuremath{\delta}a\_cell \ensuremath{\approx} 9.5 \ensuremath{\times} 10\textsuperscript{-75} m\textsuperscript{2}.

\textbf{Reverse-engineering $\alpha_s$ from the pixel-area scale.} The pixel-area relation
(Section 5.4) provides an independent route to $\alpha_s$(M\_Z). The cell area in
Planck units is:
...
This is exactly the heat-kernel form p\_R(t) \ensuremath{\propto} d\_R\^{}\ensuremath{\kappa} exp(\ensuremath{-}t C\textsubscript{2}(R)) with \ensuremath{\kappa} = 1 in the simplest MaxEnt edge state.
...
Using the measured \(a_{\mathrm{cell}}/\ell_p^2 = 1.63094\) and \ensuremath{\alpha}\textsubscript{2}(M\_Z) \ensuremath{\approx} 0.0338
...
**$\alpha_s$(M\_Z)\textbar\_pixel \ensuremath{\approx} 0.1175**

This is consistent with the PDG EW-fit value $\alpha_s$(M\_Z) = 0.1177 \ensuremath{\pm} 0.0009 to within
...
with n\_f = 5 at M\_Z, the above $\alpha_s$ corresponds to
$\Lambda_{\overline{\mathrm{MS}}}^{(5)}$ \ensuremath{\approx} 0.208 GeV. With the lattice-motivated ansatz
m\_p \ensuremath{\approx} 4.47 $\Lambda_{QCD}$:

\[
m_p^{\mathrm{est}} \approx 4.47 \times 0.208 \simeq 0.93 \text{ GeV},
\]

within \textasciitilde1\% of the physical proton mass m\_p = 0.938 GeV, connecting pixel
geometry to hadronic physics. The factor 4.47 remains an external lattice input.

\textbf{Simultaneous prediction of $\alpha_s$ and sin\textsuperscript{2}$\theta_W$.} The pixel-area constraint can
...
**$\alpha_s$(M\_Z) = 0.1175, sin\textsuperscript{2}$\theta_W$(M\_Z) = 0.2311**
\end{quote}

\textbf{Comparison to measurement:}
- $\alpha_s$(M\_Z): predicted 0.1175 vs PDG 0.1177 \ensuremath{\pm} 0.0009, difference 2\ensuremath{\times}10\textsuperscript{-4} (within 1\ensuremath{\sigma})
- sin\textsuperscript{2}$\theta_W$(M\_Z): predicted 0.2311 vs PDG \^{s}\textsuperscript{2}\_Z = 0.23122 \ensuremath{\pm} 0.00006 (MS-bar), difference \textasciitilde1\ensuremath{\times}10\textsuperscript{-4} (\textasciitilde2\ensuremath{\sigma})

\textbf{Important caveat on sin\textsuperscript{2}$\theta_W$.} In \emph{percentage} terms, 0.05\% looks impressive. But
...
The required correction is small: shifting sin\textsuperscript{2}$\theta_W$ by \textasciitilde1\ensuremath{\times}10\textsuperscript{-4} corresponds to
only \textasciitilde0.04\% change in \ensuremath{\alpha}\textsubscript{2}(M\_Z). Effects at this level include:
...
omits. The claim is therefore: the framework produces sin\textsuperscript{2}$\theta_W$ within \textasciitilde2\ensuremath{\sigma}
...
framework cannot claim precision better than \textasciitilde0.1\% on sin\textsuperscript{2}$\theta_W$. The \textasciitilde2\ensuremath{\sigma}
...
sin\textsuperscript{2}$\theta_W$(M\_Z) as inputs. The pixel-area constraint eliminates sin\textsuperscript{2}$\theta_W$ as an
...
\begin{itemize}
\item
  \textbf{Entropy (MaxEnt selection)} traces over one side of the entanglement cut, giving the factor d\_R in the probability p\_R \ensuremath{\propto} d\_R exp(\ensuremath{-}t C\textsubscript{2}(R)).
\item
...
\textbf{Numerical result at unification.} At t\_U \ensuremath{\approx} 1.64 (corresponding to $\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 24.1):
...
\textbf{Numerical outputs.} Using the Peter-Weyl-derived beta shifts and measured $\alpha_{em}$(M\_Z), sin\textsuperscript{2}$\theta_W$(M\_Z) to fix \ensuremath{\alpha}\textsubscript{1}, \ensuremath{\alpha}\textsubscript{2}, the edge mechanism predicts:

\begin{itemize}
\tightlist
\item
  $\alpha_s$(M\_Z) \ensuremath{\approx} 0.1168
\item
  M\_U \ensuremath{\approx} 2.0 \ensuremath{\times} 10\textsuperscript{16} GeV
\item
  $\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 24.3
\end{itemize}

Using 4-loop MS running with n\_f = 5, this $\alpha_s$ corresponds to:

$\Lambda_{MS}$\textsuperscript{(5)} \ensuremath{\approx} 195 MeV
...
conversion constant C\_p (essentially what lattice QCD computes), but the
...
- Q\_L: (3, 2, Y=1/6) =\textgreater{} (\ensuremath{\tau}=1, j=1/2, n=1) \checkmark{}
- L\_L: (1, 2, Y=\ensuremath{-}1/2) =\textgreater{} (\ensuremath{\tau}=0, j=1/2, n=\ensuremath{-}3) \checkmark{}
...
\textbf{Heat-kernel slopes at M\_Z.} The general relation is t = g\textsuperscript{2}A\_eff/2 where
A\_eff is the effective ``Euclidean thickness'' of the collar. Section 6.14's
SU(3) lattice analysis finds A\_eff \ensuremath{\to} 2.004 \ensuremath{\pm} 0.012 as g\textsuperscript{2} \ensuremath{\to} 0.

Using g\textsuperscript{2} = 4\ensuremath{\pi}\ensuremath{\alpha} and defining the \textbf{normalization convention} A\_eff = 4\ensuremath{\pi}
...
t\_i = (g\_i\textsuperscript{2} \ensuremath{\cdot} 4\ensuremath{\pi})/2 = 2\ensuremath{\pi} \ensuremath{\cdot} g\_i\textsuperscript{2} = 4\ensuremath{\pi}\textsuperscript{2} $\alpha_i$

With the electroweak inputs and PDG 2025 EW-fit value $\alpha_s$(M\_Z) = 0.1177:
...
\emph{(Note: Using the MSSM unification prediction $\alpha_s$ = 0.1166 instead would give
t\_3 = 4.605, a 1.2\% shift that negligibly affects the residue-class distributions
below.)}

For the U(1) factor with Y = n/6, the effective slope is t\_Y = t\textsubscript{1}/36 = 0.0186.

\emph{Normalization note:} The A\_eff = 4\ensuremath{\pi} convention gives t = 4\ensuremath{\pi}\textsuperscript{2}\ensuremath{\alpha}, a clean
relation used throughout. However, Section 6.14's lattice extrapolation
gives A\_eff \ensuremath{\to} 2, not 4\ensuremath{\pi} \ensuremath{\approx} 12.6. This factor-of-\textasciitilde6 discrepancy indicates
...
\textbf{Hypercharge residue class distribution at M\_Z.} Summing over allowed
...
At M\_Z, because SU(3) is strongly coupled (t\textsubscript{3} large), triality-zero
...
S\_r(t\_Y) := $\Sigma_k$ exp(\ensuremath{-}t\_Y(6k+r)\textsuperscript{2})

By Poisson summation, the relative deviation between residue sums is \textasciitilde2 exp(\ensuremath{-}\ensuremath{\pi}\textsuperscript{2}/(36 t\_Y)). At M\_Z with t\_Y = 0.0186:

max\_r \textbar( S\_r \ensuremath{-} S ) / S\textbar{} \ensuremath{\approx} 7.8 \ensuremath{\times} 10\textsuperscript{-7}
...
\textbf{Numerical result.} Computing the edge entropy S\_edge = H(p\_\ensuremath{\alpha}) + \ensuremath{\langle}log d\_\ensuremath{\alpha}\ensuremath{\rangle}:
...
\item
  S\_edge\^{}Z\textsubscript{6}(M\_Z) = 4.000 bits
\end{itemize}

The deficit is:

\ensuremath{\Delta}S(M\_Z) = 2.58497 bits \ensuremath{\approx} log\textsubscript{2} 6

The deviation from log\textsubscript{2} 6 is \textasciitilde4 \ensuremath{\times} 10\textsuperscript{-6} bits.

\textbf{Scale dependence.} At the unification scale (t\_U \ensuremath{\approx} 1.64 for all
factors), nontrivial SU(3) triality sectors become more probable:

\begin{itemize}
\tightlist
\item
  r \ensuremath{\equiv} 0 (mod 6): P = 0.606 at M\_Z, P = 0.383 at M\_U
\item
  r \ensuremath{\equiv} 3 (mod 6): P = 0.382 at M\_Z, P = 0.204 at M\_U
\item
  r \ensuremath{\equiv} 2 or 4 (mod 6): P = 0.004 at M\_Z, P = 0.134 at M\_U
\item
  r \ensuremath{\equiv} 1 or 5 (mod 6): P = 0.002 at M\_Z, P = 0.072 at M\_U
\end{itemize}
...
- Numerically precise to 10\textsuperscript{-6} bits at M\_Z
...
$\alpha_s$ \ensuremath{\to} $\Lambda_{QCD}$.
...
arises by dimensional transmutation, just as $\Lambda_{QCD}$ arises from $\alpha_s$.

\textbf{Setup.} From the pixel-area relation (Section 5.4):
- a\_cell/$\ell_p$\textsuperscript{2} = 1.63094
- \ensuremath{\xi}/$\ell_p$ = \ensuremath{\sqrt{}}1.63094 = 1.2771
- E\_cell = E\_p / (\ensuremath{\xi}/$\ell_p$) = 9.56 \ensuremath{\times} 10\textsuperscript{18} GeV

\textbf{Transmutation ansatz.} Assume EWSB is triggered by an edge-sector ordering transition whose scale is set by dimensional transmutation from the UV cell scale, with a one-loop coefficient $\beta_{EW}$ controlled by the same edge-mode content that produces the MSSM-like beta-function shift:

v = E\_cell \ensuremath{\cdot} exp(\ensuremath{-}2\ensuremath{\pi} / ($\beta_{EW}$ \ensuremath{\cdot} $\alpha_U$))

The edge-mode computation gives \ensuremath{\Delta}b\textsubscript{3} \ensuremath{\approx} 4.00 (Section 6.17). This integer has a structural origin: $\beta_{EW}$ = N\_c + 1 = 4 is the number of SU(2) doublets per generation (N\_c quark doublets plus one lepton doublet). This is not a fit parameter; it is a topological/anomaly-counting integer already derived in Section 6.9 from the Witten anomaly constraint.

\textbf{Computation.} Using $\alpha_U$\textsuperscript{-1} = 24.32 from the unification analysis:

2\ensuremath{\pi} / ($\beta_{EW}$ \ensuremath{\cdot} $\alpha_U$) = 2\ensuremath{\pi} / (4 \ensuremath{\times} 0.0411) = 38.21
...
\begin{quote}
\textbf{Prediction (Electroweak scale): v\_pred \ensuremath{\approx} 243.5 GeV}
\end{quote}

\textbf{Comparison to measurement.} The measured Higgs VEV is v\_obs \ensuremath{\approx} 246.2 GeV.
...
$\beta_{EW}$ = 2\ensuremath{\pi} / ($\alpha_U$ \ensuremath{\cdot} ln(E\_cell/v\_obs)) \ensuremath{\approx} 4.001

The coefficient demanded by Nature is $\beta_{EW}$ = 4 to within \textasciitilde0.03\%. This is precisely the integer that appears in the gauge-sector beta-function shift.

\textbf{Caveat.} The structural argument (N\_c + 1 doublets) provides a rationale for $\beta_{EW}$ = 4, but it is also the integer that fits the data. The claim that this is ``derived'' rather than ``fitted'' rests on whether the anomaly-counting argument is accepted as fundamental. Skeptics may view this as choosing the integer that works.

\subsubsection{Top quark mass from order-one Yukawa}\label{top-quark-mass-from-order-one-yukawa}

If the top Yukawa is order-one (the natural MaxEnt/refinement-stability outcome for the least-suppressed Yukawa channel), then y\_t \ensuremath{\approx} 1 and:

\begin{quote}
\textbf{Prediction (Top quark mass): m\_t \ensuremath{\approx} v/\ensuremath{\sqrt{}}2 \ensuremath{\approx} 172.2 GeV}
\end{quote}

The measured top mass is m\_t \ensuremath{\approx} 172.7 GeV, so the prediction is \textbf{\textasciitilde0.3\% low}.

\textbf{Caveat.} This is not a genuine prediction. The assumption ``y\_t \ensuremath{\approx} 1'' is an empirical fact; it's what makes the top quark special. The ``derivation'' restates observation rather than predicting it. A genuine prediction would derive y\_t \ensuremath{\approx} 1 from first principles, which the framework does not do.
...
\textbf{y\_f \ensuremath{\propto} $\varepsilon^n_f$ = 6\^{}(\ensuremath{-}n\_f), where \ensuremath{\varepsilon} = 1/6}
...
\textbf{Extraction of defect charges.} Using y\_f = \ensuremath{\sqrt{}}2 \ensuremath{\cdot} m\_f / v\_pred and n\_f = \ensuremath{-}ln(y\_f) / ln(6):
...
fingerprint.
2. The residual coefficients c\_f are all order-one (0.6--2.2), consistent with
...
(-1.1\% error); the reverse-engineered $\beta_{EW}$ = 4.00116 matches the integer 4
to 0.03\% precision.
...
\textbf{The critical surface constraint.} Refinement stability pushes the scalar potential to a marginal stability point at the matching scale \ensuremath{\mu}* = M\_U. The sharpest encoding of ``marginally stable'' is:

\ensuremath{\lambda}(M\_U) = 0, \ensuremath{\beta}\_\ensuremath{\lambda}(M\_U) = 0
...
\ensuremath{\beta}\_\ensuremath{\lambda} \ensuremath{\propto} \ensuremath{-}6 y\_t\textsuperscript{4} + (3/8)(2g\textsubscript{2}\textsuperscript{4} + (g\textsubscript{2}\textsuperscript{2} + g\textsubscript{1}\textsuperscript{2})\textsuperscript{2})

Setting \ensuremath{\beta}\_\ensuremath{\lambda}(M\_U) = 0 immediately fixes y\_t(M\_U) in terms of the gauge couplings:

y\_t(M\_U) = {[}(1/16)(2g\textsubscript{2}\textsuperscript{4} + (g\textsubscript{2}\textsuperscript{2} + g\textsubscript{1}\textsuperscript{2})\textsuperscript{2}){]}\^{}(1/4)
...
\ensuremath{\mu}* = M\_U \ensuremath{\approx} 2.08 \ensuremath{\times} 10\textsuperscript{16} GeV

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Run g\textsubscript{1}, g\textsubscript{2}, g\textsubscript{3} from M\_Z up to M\_U at one loop in the SM:

  \begin{itemize}
  \tightlist
  \item
    g\textsubscript{1}(M\_U) \ensuremath{\approx} 0.5794
  \item
    g\textsubscript{2}(M\_U) \ensuremath{\approx} 0.5213
  \item
    g\textsubscript{3}(M\_U) \ensuremath{\approx} 0.5265
  \end{itemize}
\item
  From \ensuremath{\lambda} = 0, \ensuremath{\beta}\_\ensuremath{\lambda} = 0: y\_t(M\_U) \ensuremath{\approx} 0.4239
\item
  Run (g\_i, y\_t, \ensuremath{\lambda}) back down to \ensuremath{\mu} = M\_t \ensuremath{\approx} 173 GeV:

  \begin{itemize}
  \tightlist
  \item
    y\_t(M\_t) \ensuremath{\approx} 0.9192
  \item
    \ensuremath{\lambda}(M\_t) \ensuremath{\approx} 0.1290
  \end{itemize}
\item
  Convert to Higgs mass using m\_H = \ensuremath{\sqrt{}}(2\ensuremath{\lambda}(M\_t)) \ensuremath{\cdot} v:
\end{enumerate}

\begin{quote}
\textbf{Prediction (Higgs mass): m\_H \ensuremath{\approx} 125.08 GeV}
\end{quote}

\textbf{Comparison to measurement.} The measured Higgs mass is m\_H\^{}obs = 125.09 \ensuremath{\pm} 0.24 GeV. The prediction matches to \textbf{within 0.01 GeV}, essentially exact agreement.

\textbf{Significance.} This is not a fit to m\_H. It emerges from:
1. The unification scale M\_U (already determined by the pixel-area gauge coupling pipeline)
2. The refinement-stability constraint \ensuremath{\lambda} = \ensuremath{\beta}\_\ensuremath{\lambda} = 0 at M\_U
...
m\_t\^{}MS(M\_t) = y\_t(M\_t) \ensuremath{\cdot} v / \ensuremath{\sqrt{}}2 \ensuremath{\approx} (0.9192 \ensuremath{\times} 246.22) / \ensuremath{\sqrt{}}2 \ensuremath{\approx} 160.0 GeV

The pole mass is higher after QCD/EW threshold corrections, consistent with the observed m\_t \ensuremath{\approx} 172.7 GeV pole mass.

\textbf{Chain summary.} The critical surface constraint closes the loop: pixel area \ensuremath{\to} M\_U \ensuremath{\to} (\ensuremath{\lambda} = 0, \ensuremath{\beta}\_\ensuremath{\lambda} = 0) \ensuremath{\to} y\_t(M\_U) \ensuremath{\to} RG evolution \ensuremath{\to} m\_H \ensuremath{\approx} 125 GeV.
...
This is a derived relation, not an assumption. However, the \textbf{numerical value} of a\_cell/$\ell_p$\textsuperscript{2} depends on the t\_i values, which depend on the couplings.
...
\textbf{Result.} At t\_U \ensuremath{\approx} \ensuremath{\pi}\textsuperscript{2}/6 \ensuremath{\approx} 1.645 (corresponding to $\alpha_U$\textsuperscript{-1} \ensuremath{\approx} 24), with one overall normalization fixed by demanding \ensuremath{\Delta}b\textsubscript{2} matches MSSM:
...
- \ensuremath{\alpha}\textsuperscript{-1}(M\_Z) = 127.951 \ensuremath{\pm} 0.009
- \^{s}\textsuperscript{2}\_Z = sin\textsuperscript{2}$\theta_W$(M\_Z) = 0.23122 \ensuremath{\pm} 0.00004
- $\alpha_s$(M\_Z) = 0.1180 \ensuremath{\pm} 0.0009

The piecewise running (SM below M\_S, edge-UV above) gives a 2\ensuremath{\times}2 system for x = ln(M\_S/M\_Z) and y = ln(M\_U/M\_S). Solution:
...
\textbf{Inputs:}
1. Pixel constant: a\_cell/$\ell_p$\textsuperscript{2} = 1.631 (treat as fundamental)
2. One electroweak datum: \ensuremath{\alpha}\textsuperscript{-1}(M\_Z) = 127.951

\textbf{Constraints:}
- Pixel: a\_cell/$\ell_p$\textsuperscript{2} = 4(\ensuremath{\ell}\textsubscript{2} + \ensuremath{\ell}\textsubscript{3})
...
- $\alpha_s$(M\_Z): predicted 0.1175 vs measured 0.1180 \ensuremath{\pm} 0.0009 \ensuremath{\to} 0.6\ensuremath{\sigma} low
- sin\textsuperscript{2}$\theta_W$: predicted 0.2310 vs measured 0.23122 \ensuremath{\pm} 0.00004 \ensuremath{\to} \textasciitilde2\ensuremath{\sigma}

The $\alpha_s$ agreement is excellent. The sin\textsuperscript{2}$\theta_W$ tension (\textasciitilde2\ensuremath{\sigma}) is where precision threshold/two-loop effects matter.

\textbf{Step 7: Consistency check, beta\_EW from v.}

If the pixel constant P and electroweak VEV v are both treated as inputs, we can solve for the transmutation coefficient $\beta_{EW}$ that reproduces v:

\[
v = \frac{E_p}{\sqrt{P}} \exp\left(-\frac{2\pi}{\beta_\mathrm{EW} \cdot \alpha_U}\right)
\]

Using P = 1.63094, v = 246.22 GeV, E\_p = 1.22089 \ensuremath{\times} 10\textsuperscript{19} GeV:

\[
\beta_\mathrm{EW}^\mathrm{req} = 3.997
\]

This is $\beta_{EW}$ = 4 to within 0.1\%. The integer 4 = N\_c + 1 (number of SU(2) doublets per generation) emerges from fitting, but also has a structural rationale from the Witten anomaly constraint.
...
Numerical value of a\_cell/$\ell_p$\textsuperscript{2} & \textbf{Not derived}; requires fixing t (hence \ensuremath{\alpha}) \\
...
$\beta_{EW}$ = 4 & Structural (N\_c + 1) or fitted; ambiguous status \\
\end{longtable}
}
...
\item
  Treating a\_cell/$\ell_p$\textsuperscript{2} as fundamental input (replaces one coupling)
\item
...
\item
  \textbf{Newton's constant formula}: G = a\_cell / (4 \ensuremath{\ell}(t)) from edge entropy density (Section 5.4)
\item
  \textbf{Discrete area spectrum}: Log-integer area eigenvalues from edge sectors (Section 5.11); this is robust
\item
  \textbf{Discrete Hawking/GW comb}: The specific comb pattern \ensuremath{\Delta}E\_k = k\_B T\_H ln(k) follows if integer-multiplication transitions dominate; generic transitions would give a denser log-rational spectrum
\end{itemize}
...
\item
  \textbf{Yukawa hierarchy y\_f \ensuremath{\propto} 6\^{}\{-n\_f\}}: Z\textsubscript{6} defect suppression with integer charges
\end{itemize}
...
\tightlist
\item
  \textbf{Strong coupling}: $\alpha_s$(M\_Z) \ensuremath{\approx} 0.1175 vs PDG 0.1177 \ensuremath{\pm} 0.0009 (within 1\ensuremath{\sigma})
\item
  \textbf{Weak mixing angle}: sin\textsuperscript{2}$\theta_W$ \ensuremath{\approx} 0.2311 vs PDG \^{s}\textsuperscript{2}\_Z = 0.23122 \ensuremath{\pm} 0.00006 (MS-bar, \textasciitilde2\ensuremath{\sigma})
\item
  \textbf{Z\textsubscript{6} charge quantization}: PDG bounds confirm \textbar q\_p + q\_e\textbar/e \textless{} 10\textsuperscript{-21}, fractional charge abundance \textless{} 10\textsuperscript{-22}/nucleon, CMS excludes fractionally charged particles to 640 GeV (Section 6.12)
\item
...
\item
  \textbf{Photon mass}: PDG bound m\_\ensuremath{\gamma} \textless{} 10\textsuperscript{-18} eV confirms exact zero
\item
  \textbf{Graviton mass}: PDG bound m\_g \textless{} 1.76 \ensuremath{\times} 10\textsuperscript{-23} eV confirms exact zero
\end{itemize}
...
\item
  \textbf{SM global gauge group \(SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\)}: MAR + admissibility (GAUGE\_GROUP\_DERIVATION.md)
\item
...
\tightlist
\item
  \textbf{$\alpha_s$(M\_Z) \ensuremath{\approx} 0.117 with MSSM spectrum}: MSSM GUT analyses (1990s)
\item
  \textbf{sin\textsuperscript{2}$\theta_W$(M\_U) = 3/8}: Georgi and Glashow (1974)
\item
...
where $\Delta_R$ = ln(p\textsubscript{0}/d\textsubscript{0}) \ensuremath{-} ln(p\_R/d\_R) = t C\textsubscript{2}(R).
...
\tightlist
\item
  \textbf{(c\_GW \ensuremath{-} c)/c}: Model predicts = 0 exactly. Bound: {[}\ensuremath{-}3\ensuremath{\times}10\textsuperscript{-15}, +7\ensuremath{\times}10\textsuperscript{-16}{]}
\item
...
f\_\{k,m\}(M,\ensuremath{\chi}) = (m $\Omega_H$)/(2\ensuremath{\pi}) + (c\textsuperscript{3} g(\ensuremath{\chi}))/(16\ensuremath{\pi}\textsuperscript{2} GM) \ensuremath{\cdot} ln(k) for k = 2, 3, 4, \ldots{}

After rescaling by remnant parameters, all events should stack at universal coordinates x\_k = ln(k)/(8\ensuremath{\pi}). This is checkable with public LIGO/Virgo data. Absence of coherent stacking at the predicted x\_k would falsify the log-integer area spectrum.

\textbf{Discrete Hawking comb (Section 5.11).} For primordial black holes in the final evaporation stage, gamma-ray bursts should show comb structure at E\_k/E\textsubscript{2} = ln(k)/ln(2). Current PBH burst searches (Fermi, H.E.S.S.) can constrain this with dedicated template analysis.
...
\item
  \emph{Mass-independent fractional linewidth}: The intrinsic linewidth \ensuremath{\Gamma}/\ensuremath{\Delta}E\_k \ensuremath{\approx} 3-5\% is approximately independent of black hole mass. This is a sharp shape prediction constraining not just line positions but line profiles.
\item
...
\textbar\ensuremath{\langle}\ensuremath{\Delta}K\ensuremath{\rangle}\textbar{} \ensuremath{\leq} 2\textbar A\textbar{} \ensuremath{\cdot} $\eta^{w/\ensuremath{\xi}}$
...
\item
  ...
  y\_f \ensuremath{\propto} 6\^{}\{\ensuremath{-}n\_f\} with integer defect charges n\_f.~What remains: derive the
...
\item
  ...
  derives \ensuremath{\Delta}b \ensuremath{\approx} (2.49, 4.38, 3.97) from the heat-kernel distribution at t\_U \ensuremath{\approx} 1.64,
  matching MSSM targets to within 5\%. What remains: derive t\_U from group-theoretic
...
\item
  ...
  the UV and which operator generates v with coefficient $\beta_{EW}$ = N\_c + 1 = 4.
  Motivated by refinement stability, but not yet dynamically derived.
\item
  \textbf{Higgs mass m\_H}: Requires the quartic \ensuremath{\lambda} (or MSSM threshold matching).
\item
  \textbf{$\theta_{QCD}$} (strong CP problem): See program lemma below.
\item
...
\textbf{Proposition (Local modular data cannot fix \ensuremath{\Lambda}).} Any reconstruction of T\_ab from null modular generators determines it only up to \ensuremath{\varphi} g\_ab. Consequently, the Einstein equation derived from local entanglement equilibrium is fixed only up to \ensuremath{\Lambda} g\_ab, and \ensuremath{\Lambda} must be fixed by a global constraint or reference state choice.

In 4D de Sitter with horizon radius r\_dS = \ensuremath{\sqrt{}}(3/\ensuremath{\Lambda}):
- Horizon area: A\_dS = 4\ensuremath{\pi} r\_dS\textsuperscript{2} = 12\ensuremath{\pi}/\ensuremath{\Lambda}
- de Sitter entropy: S\_dS = A/(4G) = 3\ensuremath{\pi}/(G\ensuremath{\Lambda})

If the fundamental screen Hilbert space has finite total dimension dim(H\_tot) = exp(S\_dS), then:

\textbf{\ensuremath{\Lambda} = 3\ensuremath{\pi} / (G \ensuremath{\cdot} log dim H\_tot)}

\textbf{Interpretation.} \ensuremath{\Lambda} is not determined by local physics; it is the global ``capacity'' parameter of the static patch, set by the total number of microscopic degrees of freedom on the screen. This explains why \ensuremath{\Lambda} is hard to predict: it requires knowing dim(H\_tot), which depends on UV details not fixed by the axioms. The observed small value implies log(dim H) \textasciitilde{} 10\textsuperscript{122}.

\textbf{Program lemma for $\theta_{QCD}$.} In 3+1D, a \ensuremath{\theta}-term is a topological angle. In the gluing/obstruction language, \ensuremath{\theta} corresponds to a nontrivial 2-group cocycle on triple overlaps:

\textbf{Conjecture (\ensuremath{\theta} as gluing obstruction).} Adding a \ensuremath{\theta}-term corresponds to weighting gauge histories by exp(i\ensuremath{\theta}Q). On the screen net, this appears as a nontrivial 2-cocycle (g\_ij, h\_ijk) whose 4D extension class is nonzero. If loop-coherent gluing is imposed (vanishing obstruction in the appropriate cohomology), then \ensuremath{\theta} is forced to a discrete set \{0, \ensuremath{\pi}\} (CP-even points). Refinement stability + MaxEnt then selects \ensuremath{\theta} = 0 unless CP is spontaneously broken.

\textbf{Status.} This is a derivation target, not a proven result. If correct, it
would explain why $\theta_{QCD}$ \ensuremath{\approx} 0 without fine-tuning: the same consistency
...
\emph{How OPH addresses it:} The framework combines MaxEnt entanglement equilibrium, the derived K\_C = 2\ensuremath{\pi}B\_C structure with rigid normalization, and an EFT bridge identifying modular energy with stress-tensor charges. The null modular additivity route (N1-N3 derived from Markov/edge-center mechanisms on null strips) internalizes the EFT bridge rather than importing ``assume a UV CFT.''
...
\emph{How OPH addresses it:} From null modular data, T\_ab is reconstructed only up to \ensuremath{\varphi}g\_ab. Local consistency conditions and null focusing are blind to vacuum-energy shifts, so the Einstein equation is fixed only up to \ensuremath{\Lambda}g\_ab. \ensuremath{\Lambda} becomes a global ``capacity'' parameter of the static patch (tied to log dim H\_tot), not a locally computable quantity. This dissolves a conceptual tension: local microphysics \emph{cannot} fix \ensuremath{\Lambda} by structural information-theoretic reasons.
