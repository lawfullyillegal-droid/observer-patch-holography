...
Charged leptons & continuation branch & centered charged-shape class and same-carrier readback & theorem-grade waiting set is branch-generator splitting to promote \(\widehat C_e^{\mathrm{cand}}\), then the refinement-stable uncentered trace lift beneath the determinant-line route, then one affine-covariant absolute anchor \(A_{\mathrm{ch}}\) \\
Neutrinos & weighted-cycle continuation & PMNS/hierarchy shape and the scale-free mass normal form & one residual bridge scalar \(B_\nu\) remains open, with \(\lambda_\nu\) induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization; no further discrete branch object remains on that lane \\
