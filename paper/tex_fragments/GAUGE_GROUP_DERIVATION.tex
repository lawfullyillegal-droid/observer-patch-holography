...
\textbf{Abstract.} We show that, once Tannaka/DR reconstruction is applied to the refinement-stable edge-sector colimit, the extended OPH theory , core axioms A1--A4, regulator premises R0--R1, the loop-coherent gluing condition \([z]=0\), and the explicit structural-economy selector MAR (Minimal Admissible Realization) , select the Standard Model connected Lie gauge-sector factors, the number of colors, and the number of generations once the admissible class includes one connected abelian charge factor. The realized hypercharge lattice then fixes the global gauge quotient:
