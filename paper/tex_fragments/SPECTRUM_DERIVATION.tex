...
The entire prediction pipeline uses exactly \textbf{two} physical inputs:
...
\bottomrule\noalign{}
\endlastfoot
Pixel area constant & \(P \equiv a_{\text{cell}}/\ell_P^2\) & 1.63094 & Edge entropy matching (§5.4 of Part I of this manuscript) \\
...
\subsubsection{1.3 No-Cheat Guarantee}\label{13-no-cheat-guarantee}

The prediction code enforces a runtime mutation test: after computing all predictions, the PDG reference table is scrambled and predictions are recomputed. Any change triggers an assertion failure. This is implemented in \texttt{oph\_predict\_compare.py::\_assert\_pdg\_not\_used()}.
...
\(\varepsilon\) (Z₆ defect) & \(1/6\) & \texttt{defect\_epsilon\_Z6()} & The SM gauge group has a \(\mathbb{Z}_6\) center quotient. Each unit of \(\mathbb{Z}_6\) defect insertion removes \(\Delta S = \ln 6\) nats of entropy (MaxEnt weighting, Assumption B). The resulting Boltzmann suppression per defect is \(e^{-\ln 6} = 1/6\). \textbf{This is topologically fixed} by the quotient structure, not chosen. & §6.18, §6.21 \\
...
Quark exponents & \(n_u = (6,3,0)\), \(n_d = (6,4,2)\) & \texttt{derive\_integer\_vectors()} & Minimal SU(3) hierarchy structure: \(n_{u} = (2N_c, N_c, 0)\) and \(n_d = (2N_c, N_c+1, N_c-1)\). These are the unique sequences that (a) span the range \([0, 2N_c]\) in \(N_g\) steps, (b) have the top quark unsuppressed (\(n_t = 0\)), and (c) match the observed up/down mass ordering under CKM rotation. With \(N_c = 3\): \((6,3,0)\) and \((6,4,2)\). & §6.21 \\
Lepton exponents & \(n_e = (7,4,3)\) & \texttt{derive\_lepton\_exponents()} & Derived algorithmically: the Koide roots \(r_k^2\) (from \(\delta = 2/9\)) must satisfy \(r_i^2/r_j^2 \approx \varepsilon^{n_i - n_j}\) for all pairs. The code scans integer triples near \((N_g, N_g+1, \ldots)\) and selects the unique triple minimizing the log-ratio residuals. \textbf{Result}: \((n_\tau, n_\mu, n_e) = (3, 4, 7)\). & §6.21, §13 of Part V of this manuscript \\
CKM angles & \(s_{12} = \varepsilon\), \(s_{23} = \varepsilon^2\), \(s_{13} = \varepsilon^3\) & inline & Powers of \(\varepsilon = 1/6\): the CKM mixing angles scale as \$\textbackslash varepsilon\^{}\{ & n\_i - n\_j \\
\(Q\) (Koide ratio) & \(2/3\) & inline & The \(\mathbb{Z}_3\) mode balance on generation space: the circulant Hermitian matrix \(\Phi = a I + b P + b^* P^2\) on 3 generations satisfies \$Q = (1 + 2 & b/a \\
\end{longtable}
}
...
SM 1-loop \(\beta\)-coefficients & \((b_1, b_2, b_3)^{\text{SM}} = (41/10, -19/6, -7)\) & \texttt{B\_SM\_1LOOP} & Standard 1-loop coefficients for the SM with \(N_g = 3\), used only for the critical-surface RG evolution from \(M_U\) to \(m_t\) (§5 of this paper). Determined entirely by the SM gauge group and matter content. \\
4-loop MSbar \(\beta\)-coefficients & \(\beta_0, \beta_1, \beta_2, \beta_3\) & \texttt{beta\_coeffs\_msbar(n\_f)} in \texttt{oph\_qcd.py} & Standard perturbative QCD: \(\beta_0 = 11 - 2n_f/3\), \(\beta_1 = 102 - 38n_f/3\), etc. These are computed from SU(3) Feynman diagrams at each loop order. The 4-loop coefficient \(\beta_3\) involves \(\zeta(3)\); all are universal in the MSbar scheme. \\
Pole-mass conversion & \(K_2 = 13.44 - 1.04 n_l\), \(K_3 = 190.6 - 26.7 n_l + 0.65 n_l^2\) & \texttt{top\_pole\_from\_msbar()} & Standard 3-loop QCD relation between \(\overline{\text{MS}}\) and pole mass (Chetyrkin, Kniehl, Steinhauser 2000). These are perturbative coefficients computed from Feynman diagrams; they depend only on \(n_l\) (number of light flavors) and SU(3) group factors. \\
Casimir eigenvalues & \(C_2(p,q)\), \(d_{(p,q)}\) & \texttt{ellbar\_su3()}, \texttt{ellbar\_su2()} & Group-theoretic: \(C_2(p,q) = \frac{1}{3}(p^2 + q^2 + pq + 3p + 3q)\) for SU(3), \(C_2(j) = j(j+1)\) for SU(2), \(d_{(p,q)} = \frac{1}{2}(p+1)(q+1)(p+q+2)\). These are properties of the Lie algebra, not physical inputs. \\
\(\zeta(3)\) & 1.20206... & \texttt{z3} in \texttt{oph\_qcd.py} & Ap\'{e}ry\textquotesingle s constant , a pure mathematical constant appearing in 4-loop \(\beta_3\). \\
...
\textbf{The single genuine physical input}:
...
\subsection{10. Complete Spectrum: Predictions vs PDG}\label{10-complete-spectrum-predictions-vs-pdg}

All predictions below use inputs \(P = 1.63094\) and \(\log(\dim\mathcal{H}) = 10^{122}\) only. PDG values are from the 2024/2025 edition, fetched via the official \texttt{pdg} Python package.
...
\(W\) boson & 80.386 & 80.377 ± 0.012 & +0.012\% & §4 & Tree-level only; missing 1-loop EW corrections (\(\Delta r\)) would shift by \textasciitilde0.1\%. Residual is well within this \\
...
\(H\) (Higgs) & 126.48 & 125.20 ± 0.11 & +1.02\% & §5 & Critical surface uses 1-loop SM RGE only. 2-loop \(\beta_\lambda\) (especially the \(\alpha_s y_t^2\) term) shifts \(m_H\) downward by \textasciitilde1 GeV, closing most of this gap. Also sensitive to exact \(M_U\) value \\
\end{longtable}
}
...
Muon & 0.10564 & 0.10566 & −0.022\% & §6 & Same Koide structure; all three leptons shift together. QED running corrections between \(m_Z\) and \(m_\mu\) are \textasciitilde0.02\%, matching the observed offset \\
...
\bottomrule\noalign{}
\endlastfoot
up & 3.74 × 10⁻³ & 2.16 × 10⁻³ & +73\% & §6 & \textbf{\(u\)-\(d\) degeneracy}: both get exponent \(n=6\), predicting \(m_u = m_d\). Real isospin breaking (\(m_u/m_d \approx 0.46\)) requires subleading defect insertions or EM corrections not included in the minimal texture \\
...
charm & 0.808 & 1.273 & −37\% & §6 & Exponent \(n_c = 3\); PDG value is \(\overline{\text{MS}}\) at \(m_c\). Missing RG running from \(\mu \sim v\) down to \(m_c\) (a factor \textasciitilde1.3 from QCD running) plus order-one Clebsch coefficient (\(c_c \approx 1.6\)) \\
bottom & 4.847 & 4.183 & +16\% & §6 & Exponent \(n_b = 2\); PDG value is \(\overline{\text{MS}}\) at \(m_b\). Running from \(v\) to \(m_b\) reduces mass by \textasciitilde15\%, which would largely close this gap. Residual is from Clebsch factor \\
...
top (crit. surf.) & 171.1 & 172.6 & −0.87\% & §5 & Independent derivation via critical surface + 3-loop pole mass conversion. Missing 2-loop RGE and exact threshold matching at \(M_U\); expected error is \textasciitilde1\% \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\(\alpha_{\text{em}}^{-1}(m_Z)\) & 128.31 & 127.952 ± 0.009 & +0.28\% & One-loop MSSM running from \(M_U\); 2-loop corrections and threshold effects at \(M_U\) and \(M_S\) are each \textasciitilde0.1\%. Combined, these could absorb the 0.28\% offset \\
...
\(\alpha_s(m_Z)\) & 0.1183 & 0.1179 ± 0.0009 & +0.37\% & Within 0.5\(\sigma\) of PDG. One-loop running; 2-loop and threshold corrections estimated at \textasciitilde0.5\%. Excellent consistency \\
\end{longtable}
}
...
\(\Lambda_{\overline{\text{MS}}}^{(3)}\) & 0.322 GeV & \textasciitilde0.332 GeV & −3.0\% & Propagated from \(\alpha_s\) error (0.4\% in \(\alpha_s\) → \textasciitilde3\% in \(\Lambda\) due to the exponential sensitivity \(d\ln\Lambda/d\ln\alpha_s \approx 7\)). Also affected by using OPH-predicted quark thresholds \(m_b, m_c\) (which differ from PDG by 16\%/37\%) in the flavor stepping \\
\end{longtable}
}
...
\subsubsection{11.1 Excellent Agreement (\textless{} 0.1\%)}\label{111-excellent-agreement--01}
...
\subsubsection{11.2 Good Agreement (0.1\% -- 2\%)}\label{112-good-agreement-01--2}

\textbf{Higgs} (+1.0\%), \textbf{top pole} (−0.87\%), \textbf{\(\alpha_s\)} (+0.37\%), \textbf{\(\sin^2\theta_W\)} (−0.21\%), \textbf{\(\alpha_{\text{em}}^{-1}\)} (+0.28\%):

The Higgs and top masses come from the critical surface constraint with 1-loop RG running. The \textasciitilde1\% error is consistent with the expected size of 2-loop corrections, threshold effects at \(M_U\), and scheme matching between the entanglement-defined coupling and \(\overline{\text{MS}}\). These is systematically improved by extending to 2-loop RGEs.

The coupling predictions at \(m_Z\) are similarly limited by one-loop running. The \(\sin^2\theta_W\) tension (\textasciitilde2\(\sigma\) relative to the precise PDG value) is where precision threshold and two-loop effects matter most.
...
No other dependencies are needed. The \texttt{pdg} and \texttt{pandas} packages are only required for \texttt{tools/fetch\_pdg\_data.py} (the PDG data fetcher, used for comparison only).
...
\bottomrule\noalign{}
\endlastfoot
\href{../code/particles/particle_masses_stage5.py}{\texttt{particle\_masses\_stage5.py}} & Core spectrum prediction: gauge closure, transmutation, critical surface, Z₆ texture & 1--5 \\
\href{../code/particles/oph_qcd.py}{\texttt{oph\_qcd.py}} & 4-loop MSbar β-coefficients and Λ extraction & 6 \\
\href{../code/particles/oph_lattice_su3_quenched_v5.py}{\texttt{oph\_lattice\_su3\_quenched\_v5.py}} & Quenched Wilson SU(3) lattice for hadron mass ratios & 6--7 \\
\href{../code/particles/oph_predict_compare.py}{\texttt{oph\_predict\_compare.py}} & Full predictor + PDG comparison (main entry point) & All \\
\href{../code/particles/oph_no_cheat_audit.py}{\texttt{oph\_no\_cheat\_audit.py}} & Static anti-leak audit of prediction code & , \\
\href{../code/particles/test_oph_predict_compare.py}{\texttt{test\_oph\_predict\_compare.py}} & Smoke tests + runtime no-cheat mutation test & , \\
\href{../code/particles/test_particle_masses_stage5.py}{\texttt{test\_particle\_masses\_stage5.py}} & Regression tests for Stage 5 predictions & , \\
\end{longtable}
}
...
\subsubsection{12.4 No-Cheat Verification}\label{124-no-cheat-verification}
...
\tightlist
\item
  \textbf{5 predictions at \textless{} 0.04\% accuracy}: \(W\), \(Z\), \(e\), \(\mu\), \(\tau\)
\item
  \textbf{5 predictions at 0.2\%--1\% accuracy}: \(H\), \(m_t^{\text{pole}}\), \(\alpha_s\), \(\sin^2\theta_W\), \(\alpha_{\text{em}}^{-1}\)
\item
  \textbf{6 quark masses} at the correct order of magnitude with hierarchy correctly reproduced, but 16\%--73\% individual errors from missing scheme matching
\item
  \textbf{3 neutrino masses} consistent with all current bounds
\item
  \textbf{3 exactly massless particles} (\(\gamma\), \(g\), graviton) from symmetry protection
\item
  \textbf{Hadron masses} from \(\Lambda_{\overline{\text{MS}}}^{(3)}\) via lattice QCD (systematic uncertainties \textasciitilde10--20\% from quenched approximation)
\end{itemize}
