...
\begin{itemize}
\item \textbf{Flavor and Yukawa data}: Yukawa matrices, flavor hierarchies, charged-lepton/Koide fits, quark-texture exponents, CKM/PMNS closure, and related downstream flavor fits. These require extra ans\"atze such as a uniform \(\mathbb Z_6\) center-label ensemble, circulant generation-space structure, discrete exponent assignments, or further external numerical machinery. The light-quark split remains a D12 continuation frontier rather than a recovered-core selector, the physical quark branch waits on a finite same-label left-handed orbit together with the provider/evaluator that exposes the first non-empty orbit elements, the charged-lepton lane fixes only the centered hierarchy until \(\widehat C_e^{\mathrm{cand}}\) is promoted and the refinement-stable uncentered trace lift beneath the determinant-line route is completed, and the neutrino continuation branch reaches the physical PMNS/hierarchy pattern while leaving the absolute spectrum one residual bridge scalar \(B_\nu\) short of closure, with the positive normalization scalar induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization. On that neutrino lane the current attached stack factors through \(q_e = q_{\mathrm{mean}} qbar_e\), so a \(qbar_e\)-only collapse law for the bridge factor does not follow from the present corpus.
...
\item \textbf{Neutrino data}: beyond the input-dependent, order-of-magnitude capacity estimate tied to $N_{\mathrm{scr}}$, the recovered core does not derive neutrino masses or mixings. A continuation branch emits intrinsic masses, splittings, and PMNS data; its weighted-cycle branch reaches the physical PMNS/hierarchy regime, the normalized same-label overlap-defect weight section closes beneath that branch, and the absolute spectrum remains one residual bridge scalar \(B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}\) short of theorem-grade closure, with the positive normalization scalar \(\lambda_\nu\) induced only after that bridge law is fixed. The attached weighted-cycle stack factors through \(q_e = q_{\mathrm{mean}} qbar_e\), so the present corpus does not derive a \(qbar_e\)-only bridge collapse law. The best closed constructive local object beneath that bridge is the defect-weighted same-label edge family \(q_e=\sqrt{g_e d_e}\) together with its induced \(\mu_e\) family, but that family does not emit \(B_\nu\) by itself.
