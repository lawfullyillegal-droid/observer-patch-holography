...
The first four steps below record the structural chain. The later threshold and two-input numerics are retained only as a historical edge-running sketch, not as the authoritative synchronized D10/D11 implementation used elsewhere in the manuscript for current quoted numbers.
...
A two-input mode of this kind is a useful illustration of how one could trade one electroweak datum for the pixel constant. The current authoritative implementation, however, is the synchronized D10/D11 package summarized elsewhere in the manuscript; full closure (option 1) still awaits a UV completion that specifies the MaxEnt constraint values.
