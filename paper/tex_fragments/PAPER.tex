...
\textbf{Proof.} Markov locality does one precise job: it reduces the modular problem to the shrinking collar, with the finite-stage deviation from an exact Markov reference carried explicitly by \(r_{\mathrm{FR}}(\varepsilon_\delta)\) and \(4\lambda_\ast^{-1}\delta^{\mathrm M}_{A_\delta:B_\delta:D_\delta}(\varepsilon_\delta)\). On the stated OPH Bisognano--Wichmann refinement branch, assumption (iii) already provides the standard cap-preserving conformal subgroup \(\lambda_C(s)\), so the limiting modular Hamiltonian must be \(K_C=\kappa B_C\) for some \(\kappa>0\). No selector-free uniqueness claim inside the cap stabilizer is used here. The modular group is KMS by definition, and because \(s\) is normalized by unit null dilation on the branch, the usual Bisognano--Wichmann relation fixes \(\kappa=2\pi\). Therefore
\[
K_C=2\pi B_C,
\]
with the displayed \(E_C^{(\delta)}\) as the carried finite-stage remainder. QED.

\textbf{Scope note.} The theorem proves the Bisognano--Wichmann conclusion \emph{once the OPH refinement branch lies in the stated BW geometric modular class}. It does \emph{not} claim that cap-preservation alone singles out a unique noncompact subgroup, nor that quasi-locality plus small collar CMI alone force the BW branch. Those remaining phase-selection burdens are stated openly rather than hidden in the theorem packaging.
