...
The exact modular-additivity identity used later is not a consequence of EC alone. It holds either for exact Markov states or in a controlled fixed-cutoff limit in which the reference state approaches the exact Markov set on one fixed collar model. This subsection therefore keeps three quantities separate:
\begin{enumerate}
...
\]
and \(\Delta K(\omega)\) is blockwise constant. Equivalently, there exists a central operator \(K_{\partial,ABD}(\omega)\in Z(\mathcal A(B))\) such that
\[
K_{ABD}(\omega)
=
...
We record an internal route toward the stress tensor that avoids assuming a UV CFT on small caps, but the sharp claim boundary is now explicit and four-layered. At fixed cutoff one can transfer the same cut-center data used in the spatial collar theorem to regulated null strips. What does not follow from that transfer alone is the left/right tensor split of the strip multiplicity spaces needed for exact splice and modular-additivity identities, nor the half-line control needed to turn strip statements into genuine translation data. The null bridge therefore separates four layers:
\begin{itemize}
\tightlist
\item
...
\item
  exact Markovity, or a controlled exact-Markov replacement on one fixed inherited strip model, yielding an exact-or-controlled four-term strip relation with an explicit carried defect operator;
\item
  endpoint-Lipschitz control for the renormalized half-line modular family, strong enough to define a weak tail generator before any Borchers promotion.
\end{itemize}
Only after all four layers are in place does the null strip support the same HJPW factorization logic as the spatial collar branch. The later promotion of the weak tail generator to a genuine positive translation generator remains the separate half-sided modular inclusion premise N2, and the later identification of that generator with a continuum null stress tensor remains a separate scaling-limit premise.
...
\item
  null-EC plus exact Markovity or a controlled exact-Markov strip limit \(\Longrightarrow\) exact-or-controlled four-term strip relation (Corollary 5.2b)
\item
  B + MX \(\Longrightarrow\) endpoint-Lipschitz interval and half-line control, hence a weak tail generator (Proposition 5.2c)
\item
  weak tail generator plus extra differentiability and tail-vanishing assumptions \(\Longrightarrow\) density formula (Lemma 5.2d)
\item
  N2 is kept explicit as the half-sided modular-inclusion premise in the scaling regime; the geometric blow-up only identifies its form, and Borchers uses it to promote the weak tail generator (Corollary 5.2e and Lemma 5.2f)
\item
...
\right\}.
\]
If the strip reference state \(\omega\) is exact Markov, then there is a central operator \(K_{\partial,J}(\omega)\in Z(\mathcal A(J))\) such that
\[
...
+\mathfrak D_J(\omega,\widetilde\omega_J),
\]
where \(K_{\partial,J}(\widetilde\omega_J):=\Delta K_J(\widetilde\omega_J)\) is central. Every bounded observable on \(I_-\cup J\) or \(J\cup I_+\) then differs from the exact-Markov reference by at most
\[
...
\]
Independently, Fawzi--Renner gives a recovered comparison state with trace-norm error \(r_{\mathrm{FR}}(\varepsilon)=2\sqrt{1-e^{-\varepsilon}}\le 2\sqrt{\varepsilon}\). Thus the exact four-term strip relation is available only at exact Markovity or in a controlled strip family on one fixed inherited strip model with \(\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon_J)\to0\), while \(\mathfrak D_J\) is carried explicitly at finite stage.

\textbf{Proof.} On the inherited split of Proposition 5.2a, exact Markovity is equivalent by HJPW to blockwise factorization of the state, so the modular Hamiltonian is additive on each block and only a direct sum of block constants remains, which is central. The controlled exact-Markov replacement is the same fixed-model compactness argument used for the spatial collar exact-Markov modulus, now with \(A,B,D\) relabeled as \(I_-,J,I_+\). The operator-logarithm Lipschitz bound then gives the displayed estimate for \(\mathfrak D_J\). Fawzi--Renner supplies a constructive comparison state but does not by itself identify an exact Markov state; that exact replacement is measured by \(\delta^{\mathrm M}_{I_-:J:I_+}\). QED.
\end{quote}

\begin{quote}
\textbf{Proposition 5.2c (Endpoint-Lipschitz null modular families and weak tail generator).} For renormalized modular Hamiltonians
\[
\widetilde K[I,\Omega]
:=
...
\]
Assumption B together with Axiom MX gives endpoint-Lipschitz control strong enough to define a weak tail generator before any Borchers theorem is invoked. More precisely, for bounded intervals \(I=(a,b)\) and \(I'=(a',b')\) in one compact endpoint window,
\[
\bigl|
\langle\psi,(\widetilde K[I',\Omega]-\widetilde K[I,\Omega])\phi\rangle
\bigr|
\le
C_{\psi,\phi,\Omega}\bigl(|a'-a|+|b'-b|\bigr),
\]
and for half-lines \(H_a=(a,\infty)\),
\[
\bigl|
...
\]
At this stage \(q(a,\Omega)\) is only a weak tail generator; positivity and unitary implementation are deferred to N2.

\textbf{Proof.} The local Gibbs form gives a quasi-local generator. Exponential mixing suppresses correlations beyond \(O(\ell_{\mathrm{UV}})\), so shifting an interval endpoint changes smeared local expectations only by \(O(\Delta v)\). For bounded intervals this yields the displayed joint endpoint-Lipschitz estimate after removing the central endpoint term. For half-lines the same estimate applies because the symmetric difference of two half-lines is the bounded interval between their endpoints. Absolute continuity of \(f_{\psi,\phi}\) then gives the weak derivative \(q\) and the integral relation above. QED.
\end{quote}

\begin{quote}
\textbf{Lemma 5.2d (Half-line density upgrade and second-derivative formula).} Fix a null generator \(\Omega\) and the renormalized half-line family \(\widetilde K_a(\Omega)\) of Proposition 5.2c. For fixed vectors \(\psi,\phi\) in a common dense domain, let
\[
f_{\psi,\phi}(a):=\langle \psi, \widetilde K_a(\Omega)\phi \rangle,
\qquad
...
-\frac{1}{2\pi}\,\partial_a f_{\psi,\phi}(a).
\]
Assume in addition that \(q_{\psi,\phi}\) is weakly differentiable in \(a\) with
\[
...
\]
so the weak tail generator admits a density upgrade precisely under these extra differentiability and tail-vanishing assumptions.
...
-\partial_a q_{\psi,\phi}(a).
\]
Integrating from \(a\) to \(+\infty\) and using \(q_{\psi,\phi}(a)\to0\) gives the formula for \(q_{\psi,\phi}(a)\); integrating once more and using \(f_{\psi,\phi}(a)\to0\) gives the displayed half-line formula. QED.
\end{quote}
...
\begin{quote}
\textbf{Lemma 5.2f (Borchers promotion to a genuine null translation generator).} Under N2, the weak tail generator of Proposition 5.2c is promoted to a genuine one-parameter unitary group \(U(a)=e^{iaP}\) with \(P\ge 0\) such that
\[
\Delta^{it}U(a)\Delta^{-it}=U(e^{-2\pi t}a),
\qquad
[K,P]=i2\pi P.
\]
\textbf{Proof.} This is the Borchers--Wiesbrock half-sided modular-inclusion theorem. Proposition 5.2c only constructs the weak tail derivative from endpoint-Lipschitz control; positivity and unitary implementation appear here, under the separate N2 premise. QED.
\end{quote}
...
\textbf{Proof.} Proposition 5.2a and Corollary 5.2b supply the precise null-strip version of collar factorization: transferred cut centers, an explicit decomposition-inheritance condition, and exact or controlled four-term modular additivity on one fixed inherited strip model, with the finite-stage failure carried by the defect operator \(\mathfrak D_J\). Proposition 5.2c defines the weak tail generator of the renormalized half-line family from endpoint-Lipschitz control. Lemma 5.2d upgrades that weak generator to a density under the additional differentiability and tail assumptions. Corollary 5.2e and Lemma 5.2f then promote the weak tail generator to the genuine positive Borchers translation generator under the separate N2 premise. The relativistic scaling-limit premise upgrades that density to \(T_{kk}\), while the three remainder sources are collected in \(E^{(\eta)}\). The null-to-tensor upgrade is the preceding lemma. QED.
...
\item
  \textbf{N1} is exact for exact Markov strip states on that inherited decomposition, and otherwise is replaced by a controlled limit with the carried defect operator \(\mathfrak D_J\);
\item
  B + MX give endpoint-Lipschitz control strong enough to define a weak tail generator for renormalized half-lines before any Borchers theorem is invoked;
\item
  N2 remains the separate promotion step to a genuine positive translation generator;
\item
  the final identification with a local continuum \(T_{kk}\) remains a scaling-limit EFT premise.
\end{itemize}
