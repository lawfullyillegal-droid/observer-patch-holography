...
\begin{quote}
OPH is an observer-centric reconstruction program for fundamental physics. From four core axioms, two external configuration inputs in the current implementation, and an explicit regulator/scaling-limit regularity package, OPH develops a conditional Jacobson-type Einstein branch, conditional compact gauge reconstruction in the bosonic edge-sector branch, and quantitative branches whose status is separated into calibration checks, independent Higgs/top outputs, and phenomenological continuations. The Standard Model, general relativity, and string-theoretic worldsheet descriptions are treated as effective sectors of that underlying structure.
\end{quote}
...
\item
  \textbf{Quantitative program from OPH inputs}: the current implementation uses the pixel area and screen capacity as external inputs. Gauge couplings and EW boson masses are calibration-sector consistency checks; the Higgs/top critical-surface branch is the main independent quantitative output; charged-lepton, flavor-texture, neutrino, and hadron branches are retained with weaker phenomenological or downstream numerical status (see Part III of this manuscript).
\item
...
\textbf{Fundamental parameters.} In the current implementation the model uses two external inputs characterizing the holographic screen:
...
Everything else (compact gauge reconstruction, charge quantization, scaling-limit Einstein equations, mass ratios) is derived structure. The axioms contain no other dimensionful constants. Quantities flowing directly from the calibrated gauge couplings are consistency checks; the Higgs/top critical-surface branch is the main independent quantitative output; Koide, charged-lepton, flavor-texture, neutrino, and related branches are retained as phenomenological continuations or downstream numerical branches and are labeled accordingly below.
