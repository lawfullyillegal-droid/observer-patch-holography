...
\textbf{Current-premise no-go theorem.} Let
\[
P:=p_a+\text{(Axioms 1--5)}+\text{Assumption 6}
+\text{the emitted OPH nodes D1--D10}
+\text{the current public D12 quark objects}.
\]
Then the present corpus does not emit \(\Theta_{ud}^{\mathrm{mass}}\), \(\Theta_{ud}^{\mathrm{phys}}\), or \(\Theta_{ud}^{\mathrm{abs}}\). The D12 mass side emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\), not a theorem-grade nonzero \(t_1\). The emitted same-label left-handed selector is fixed negatively to \(\sigma_{\mathrm{ref}}\), and same-sheet rephasing preserves CKM moduli, so the corpus emits no sector-attached same-label left-handed lift from \(\sigma_{\mathrm{ref}}\) to the physical CKM shell. The affine sector-mean split is formula-complete on the selected sheet, but the exact sextet witness remains target-anchored and \texttt{current\_family\_only}, so the corpus emits no target-free physical-sheet pair \((g_u,g_d)\). Therefore the exact same-family sextet and the D12 internal backread scalar package are sidecar representatives on their declared surfaces, not \(P\)-emitted theorem outputs.

\textbf{Minimal-extension closure theorem.} The exact minimal extension triple is
\[
H_{\mathrm{mass}}
\;:\;
