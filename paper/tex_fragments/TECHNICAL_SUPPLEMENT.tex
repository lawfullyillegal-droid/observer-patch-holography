...
This section separates one structural result from the present D12 flavor dictionary. The structural result is the realized D9 counting chain \(N_g=3\). The D12 advance is narrower: the current corpus already puts the intrinsic neutrino eta-chain, the quark mass-side continuation, and the charged same-carrier readback theorem on one common excitation base. What is not yet derived is the final charged-sector emitter, the discrete physical selector in the quark lane, or the absolute neutrino bridge scalar. Those open objects should not be repackaged as independent flavor parameters.
...
\]
This common dictionary already underlies the intrinsic neutrino eta-chain, the quark mass-side continuation, and the charged same-carrier readback theorem. The older \(6^{-n}\) suppression rule is at most the extra uniform-center-label collapse of this richer dictionary and is not a recovered-core theorem.
...
As a compare-only continuation benchmark, the base-6 logarithm lands near integers across the charged-fermion spectrum. The integer assignments themselves still depend on extra texture choices, scheme conventions, and running/matching assumptions.
...
\]
The exact theorem surface is narrower: same-carrier readback is closed, absolute scale is not, and the first missing derived object is promotion of the latent charged sector-response candidate \(\widehat C_e^{\mathrm{cand}}\) via \texttt{oph\_generation\_bundle\_branch\_generator\_splitting}. The local corpus proves neither exact vanishing nor uniform quadratic smallness of the descended commutator after the central split, so \(\widehat C_e^{\mathrm{cand}}\) remains latent only. Conditional on that promotion, the post-promotion obstruction is now smaller than the older determinant-line summary suggested: once the uncentered lift is refinement-stable on theorem-grade physical \(Y_e\), the remaining exact missing object is the descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), with \(\widetilde C_e(Y_e)=\widehat C_e(Y_e)+\mu_{\mathrm{phys}}(Y_e)\,\mathbf 1\), \(s_{\det}(Y_e)=3\mu_{\mathrm{phys}}(Y_e)\), and \(A_{\mathrm{ch}}(Y_e)=\mu_{\mathrm{phys}}(Y_e)\). A minimal extension route is still a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; internalizing that route can promote \(\widehat C_e^{\mathrm{cand}}\), but it does not by itself emit the descended physical scalar.
