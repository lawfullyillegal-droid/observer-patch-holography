...
This subsection states the precise control package needed before null modular data can be promoted to a bona fide translation generator. The logical order is
\[
\begin{aligned}
\text{center transfer}
&\Longrightarrow
\text{decomposition inheritance}
\Longrightarrow
\text{exact-or-controlled strip additivity}
\\
&\Longrightarrow
\text{endpoint-Lipschitz half-line family}
\Longrightarrow
\text{weak tail generator}
\\
&\Longrightarrow
\text{Borchers promotion under N2}.
\end{aligned}
\]

\textbf{Condition N1 (exact or controlled four-term null modular relation).} Fix a regulated null tripartition
\[
I_-=(v_1,v_2),\qquad
J=(v_2,v_3),\qquad
I_+=(v_3,v_4),
\]
with cuts \(\Gamma_-:=\{v=v_2\}\) and \(\Gamma_+:=\{v=v_3\}\). The regulator premises R0--R1 must transfer the same compact overlap-redundancy data used for ordinary collars to these two null cuts. In a compatible type-I presentation this means
\[
\tilde{\mathcal H}_{I_-}
\cong
\bigoplus_{\alpha_-}
...
W_{\alpha_+}^{*}\otimes \mathcal H_{i_+^{\alpha_+}},
\]
with opposite sides of each cut carrying inverse transport. Hence the strip algebra has the transferred center decomposition
\[
\mathcal A(J)
\cong
\bigoplus_{\alpha_-,\alpha_+}
...
\]
is the extra decomposition-inheritance condition; it is not implied by center transfer alone.

On one fixed inherited strip model, define the exact Markov set
\[
\mathfrak M_{I_-:J:I_+}
:=
\left\{
...
\right\}.
\]
If the reference state \(\omega\) is exact Markov, then
\[
...
+\mathfrak D_J(\omega,\widetilde\omega_J),
\]
where \(K_{\partial,J}(\widetilde\omega_J)\) is central. If the relevant marginals are uniformly faithful with lower spectral bound \(\lambda_\ast>0\),
\[
\|\mathfrak D_J(\omega,\widetilde\omega_J)\|_\infty
\le
4\lambda_\ast^{-1}\,\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon).
\]
This is the exact-or-controlled N1 statement. The separate Fawzi--Renner comparison error is
\[
r_{\mathrm{FR}}(\varepsilon)=2\sqrt{1-e^{-\varepsilon}}\le 2\sqrt{\varepsilon}.
\]

\textbf{Condition N2 (half-sided modular inclusion).} Null half-line algebras satisfy the half-sided modular inclusion property, either as an explicit scaling-limit premise or as the geometric blow-up form of cap modular flow near a smooth cut:
\[
\sigma_t^\omega(\mathcal A(v>v_0))
\subseteq
\mathcal A(v>v_0)
\qquad
(t\ge 0).
\]
This is the premise that promotes the weak tail generator below to a genuine positive translation generator.
...
\]
If, in addition, \(q\) is weakly differentiable and both \(f_{\psi,\phi}(a)\) and \(q_{\psi,\phi}(a)\) vanish at \(+\infty\), then the density
\[
...
\textbf{Consequence (translation generator and stress-tensor bridge).} Under N2, Borchers--Wiesbrock promotes the weak tail generator to a genuine one-parameter unitary group \(U(a)=e^{iaP}\) with \(P\ge 0\) and
\[
\Delta^{it}U(a)\Delta^{-it}=U(e^{-2\pi t}a),
\qquad
[K,P]=i2\pi P.
\]
Only after this promotion, and only with the separate relativistic scaling-limit identification premise, does the half-line density become a local null stress tensor:
\[
...
+
4\lambda_\ast^{-1}\delta_I^{\mathrm M}(\varepsilon_I)
+
\eta_I^{\mathrm{EFT}}.
\]
Thus N1 and N3 provide the fixed-cutoff control statements; N2 is the separate promotion step to a genuine positive translation generator; and the final \(T_{kk}\) identification remains a scaling-limit EFT condition rather than a fixed-cutoff theorem.
