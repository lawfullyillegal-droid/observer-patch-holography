...
\]
The exact theorem surface is narrower: same-carrier readback is closed, absolute scale is not, and the missing derived objects are first promotion of the latent charged sector-response candidate \(\widehat C_e^{\mathrm{cand}}\) via \texttt{oph\_generation\_bundle\_branch\_generator\_splitting}, then the refinement-stable uncentered trace lift beneath the determinant-line route, and only then the affine-covariant absolute anchor \(A_{\mathrm{ch}}\) on the charged common-shift quotient. The local corpus proves neither exact vanishing nor uniform quadratic smallness of the descended commutator after the central split, so \(\widehat C_e^{\mathrm{cand}}\) remains latent only. A minimal extension route is a centered Schur-type \(P\!\to\!Q\!\to\!P\) transfer theorem for the descended commutator after the central split; even if that route is internalized, the post-promotion uncentered trace lift remains a separate object before \(A_{\mathrm{ch}}\).
