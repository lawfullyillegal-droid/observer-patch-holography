...
pattern and leaves one reduced bridge-correction invariant \(C_\nu\) open above the emitted proxy \(P_\nu=I_\nu^{1/2}\widehat{\mathrm{ratio}}^{\,1/2}\mathrm{sum\_defect}^{-1}\), with the paper-facing amplitude \(B_\nu\) induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization. Hadrons are execution-contract-frozen:
...
different readout branches of the same deeper transport structure. On the current D12 surface
...
\]
The sharper post-promotion frontier is now smaller than that determinant-line summary suggested:
once \(\widehat C_e\) exists and the lift is refinement-stable on theorem-grade physical \(Y_e\), the
uncentered lift, determinant-line section, and affine anchor collapse to one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), with
\[
...
\]
is already internal to the emitted stack, the exact missing object is smaller: the reduced correction invariant
\[
C_\nu:=\frac{B_\nu}{P_\nu}.
\]
...
\]
but that family does not emit \(C_\nu\) or \(B_\nu\) by itself. The live compare-only value is \(C_\nu^\ast=0.9995473560\). A direct correction audit now gives the target-containing reduced window \(C_\nu\in[0.9994296,1.0015784]\), which induces the narrower primary corridor \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\); the older three-route bridge corridor \([6.69315,6.71259]\) remains on disk as a wider comparison envelope, and the shortlist-consensus window \([6.70484,6.71259]\) stays diagnostic-only because it misses the live compare-only target. Charged leptons and neutrinos are therefore open for different reasons: charged leptons miss promotion of \(\widehat C_e^{\mathrm{cand}}\) and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), while neutrinos miss the reduced bridge-correction invariant that would emit one positive normalization after the shape closure is already in hand.

\paragraph{Status boundary.}
The honest Stage-5 headline is therefore this: quarks have continuation-level rows; charged leptons have an exact same-carrier readback theorem together with an exact absolute-scale obstruction; neutrinos have a weighted-cycle continuation with one reduced bridge-correction invariant open. None of those later-family statements should be promoted to the recovered-core surface fixed earlier by the gauge and relativity chain.
...
free-input sector.
The forward logical order is now explicit on the live D10 surface:
\[
P
\longmapsto
...
On the sharpened D10 ledger, the paper-side transmutation factor is \(\beta_{\mathrm{transmutation,EW}}=N_c+1\); older overloaded \(\beta\)-ratios survive only on compare-only diagnostic readouts and are not part of the theorem contract.
...
Quark family & continuation & forward Yukawa derivation & up, down, strange, charm, bottom quarks & emitted same-family mass ray \(D12_{ud}^{\mathrm{mass}}\); the local same-label selector is now closed to the singleton \(\sigma_{\mathrm{ref}}\), which leaves the physical CKM-shell mismatch in place, so the next exact object on the emitted branch is the intrinsic scale law on that ray \\
Charged leptons & continuation & ordered-log and support-extension derivation & no promoted masses & centered charged shape is closed, but the theorem emits only the quotient class modulo common shift; theorem-grade waiting set is promotion of the latent candidate \(\widehat C_e^{\mathrm{cand}}\) via branch-generator splitting and then one descended physical affine scalar \(\mu_{\mathrm{phys}}(Y_e)\), from which the uncentered lift, determinant line, and affine anchor \(A_{\mathrm{ch}}\) follow canonically \\
Neutrinos & continuation & weighted-cycle PMNS/hierarchy closure & no promoted masses & one reduced bridge-correction invariant \(C_\nu\) remains open above the emitted proxy \(P_\nu\); this induces the paper-facing amplitude \(B_\nu\), the sharpened target-containing compare-only corridor is \([6.69600,6.71040]\), and absolute masses and absolute \(\Delta m^2\) remain compare-only until that bridge law is emitted \\
...
Neutrino solar splitting \(\Delta m_{21}^2\) & \(7.489806641884242\times10^{-5}\) & \(7.49\times10^{-5}\) & \(-1.93\times10^{-9}\) & \(\mathrm{eV}^2\) & compare-only absolute splitting; reduced bridge-correction law \(C_\nu\) remains open above the emitted proxy \\
...
Neutrino normal-order splitting \(\Delta m_{31}^2\) & \(2.5128980664188426\times10^{-3}\) & \(2.5129\times10^{-3}\) & \(-1.93\times10^{-9}\) & \(\mathrm{eV}^2\) & compare-only absolute splitting; reduced bridge-correction law \(C_\nu\) remains open above the emitted proxy \\
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & the current D12 sheet is a strict no-go for the physical CKM shell; the local selector is now closed to \(\sigma_{\mathrm{ref}}\), so the next exact object is the intrinsic scale law on the emitted D12 mass ray rather than another sheet-selection theorem \\
Quark family & down quark & overlap continuation & reported value \(0.00512144~\mathrm{GeV}\); stronger two-scalar candidate \(0.00470005~\mathrm{GeV}\) & same wrong-branch D12 frontier; same-sheet CKM/CP transport is closed, the local selector is now fixed to \(\sigma_{\mathrm{ref}}\), and the next honest quark object is the intrinsic scale law on the emitted D12 ray \\
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger two-scalar candidate \(0.0935045~\mathrm{GeV}\) & same wrong-branch D12 frontier; the local selector is already fixed to \(\sigma_{\mathrm{ref}}\), so the next honest quark object is the intrinsic scale law on the emitted D12 ray \\
Quark family & charm quark & overlap continuation & reported value \(1.256692~\mathrm{GeV}\); stronger two-scalar candidate \(1.272939~\mathrm{GeV}\) & same wrong-branch D12 frontier; selected-branch mass-scale fixing is now the next theorem-side object because the local selector is already fixed to \(\sigma_{\mathrm{ref}}\) \\
Quark family & bottom quark & overlap continuation & reported value \(3.820118~\mathrm{GeV}\); stronger two-scalar candidate \(4.182754~\mathrm{GeV}\) & same wrong-branch D12 frontier; the local selector is already fixed to \(\sigma_{\mathrm{ref}}\), so the remaining theorem-side quark burden is the intrinsic scale law on the emitted D12 ray, not another selector question \\
...
eventual positive lower spectral bound on each fixed local collar model.
...
Yukawa/excitation dictionary is now explicit as the common proof-facing base rather than as the
first missing object. The first remaining open front above that base is the lane-specific readout
...
at constructive intermediate objects rather than closed particle data.
...
\begin{enumerate}[leftmargin=2em]
\item the intrinsic scale law on the emitted D12 mass ray. The local same-label left-handed orbit is already closed to the singleton \(\sigma_{\mathrm{ref}}\), so the selector no longer carries the live theorem-side burden on the emitted solver surface. The mass side is already equally sharp in the negative direction: the current same-family D12 branch has an explicit underdetermination theorem saying that it emits only the one-parameter ray \(D12_{ud}^{\mathrm{mass}}\), not a theorem-grade modulus on that ray; and
...
Two open fronts remain visible and should be written exactly that way in the paper:
\begin{enumerate}[leftmargin=2em]
\item the builder-facing pure-\(B\) payload pair beneath the reported quark values is still open;
\item the broader honest overlap-continuation frontier is still a wrong-branch no-go on the current D12 sheet, but the same-label left-handed orbit is now locally closed: it collapses to the singleton \(\sigma_{\mathrm{ref}}\). That is a negative closure rather than a repaired physical branch, because the selected singleton is still the current D12 sheet. The exact next object is therefore the intrinsic scale law on the already-emitted mass-side ray \(\mathcal R_{D12}^{ud}\), not another selector theorem.
\end{enumerate}
...
splitting theorem. Conditional on that promotion, the post-promotion obstruction is now smaller
than the older determinant-line summary suggested: once the uncentered lift is refinement-stable
...
reduced bridge-correction invariant \(C_\nu\) above the emitted proxy that would emit the overall neutrino mass scale without an
external \(\Delta m_{32}^2\) anchor. Concretely, the remaining paper-facing neutrino amplitude is one
\(\lambda_\nu>0\) such that
\[
m_i=\lambda_\nu \widehat m_i,
\qquad
...
honest frontier is not a missing symbolic theorem but one production backend export bundle with publication-complete manifest provenance and real correlator arrays, followed by executed unquenched production data plus
...
\]
over the present production schedule. By contrast, the normalized backend correlator dump needed by the
...
backend export bundle that would feed the normalized dump from real production execution.
...
That is why the shared Yukawa/excitation dictionary matters so much even though it is no longer
itself the leading missing object. The flavor pipeline now exports a substantial invariant base:
...
remaining closure fronts onto one common page. The visible gates now sit above the shared
excitation dictionary rather than inside it: theorem-level charged-lepton closure, quark branch
