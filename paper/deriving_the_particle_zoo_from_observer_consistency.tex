...
pattern and leaves one residual bridge scalar \(B_\nu\) open, with the positive normalization scalar induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization. Hadrons are execution-bound:
...
\paragraph{Neutrinos.}
The weighted-cycle / Majorana-holonomy branch closes the PMNS/hierarchy shape and leaves no further discrete branch object on that lane. The normalized same-label overlap-defect weight section is already closed beneath that shape, so the remaining freedom is the positive rescaling orbit of the absolute spectrum together with the residual attachment scalar
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
above the closed normalizer. The best closed constructive local object beneath that scalar is the defect-weighted same-label edge family
\[
q_e=\sqrt{g_e d_e},
\qquad
\mu_e=\mu_\nu\,\frac{e^{\eta_e}}{\frac13\sum_f e^{\eta_f}},
\]
but that family does not emit \(B_\nu\) by itself. Charged leptons and neutrinos are therefore open for different reasons: charged leptons miss promotion of \(\widehat C_e^{\mathrm{cand}}\), the refinement-stable uncentered trace lift, and then one affine-covariant absolute anchor \(A_{\mathrm{ch}}\), while neutrinos miss the bridge scalar that would emit one positive normalization after the shape closure is already in hand.
...
Charged leptons & continuation & ordered-log and support-extension derivation & no promoted masses & centered charged shape is closed, but the theorem emits only the quotient class modulo common shift; theorem-grade waiting set is promotion of the latent candidate \(\widehat C_e^{\mathrm{cand}}\) via branch-generator splitting, then the refinement-stable uncentered trace lift, and only then the affine-covariant absolute anchor \(A_{\mathrm{ch}}\) \\
Neutrinos & continuation & weighted-cycle PMNS/hierarchy closure & no promoted masses & one residual bridge scalar \(B_\nu\) remains open; \(\lambda_\nu\) is induced only after exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization, so absolute masses and absolute \(\Delta m^2\) remain compare-only until that bridge law is emitted \\
...
Neutrino solar splitting \(\Delta m_{21}^2\) & \(7.489806641884242\times10^{-5}\) & \(7.49\times10^{-5}\) & \(-1.93\times10^{-9}\) & \(\mathrm{eV}^2\) & compare-only absolute splitting; residual bridge law \(B_\nu\) open \\
...
Neutrino normal-order splitting \(\Delta m_{31}^2\) & \(2.5128980664188426\times10^{-3}\) & \(2.5129\times10^{-3}\) & \(-1.93\times10^{-9}\) & \(\mathrm{eV}^2\) & compare-only absolute splitting; residual bridge law \(B_\nu\) open \\
...
Charged leptons & electron & continuation gap & \(n/a\); continuation pair \((\eta,\sigma)=(-6.72959,\,8.15406)\) gives near-exact centered-log shape closure, but the absolute scale remains quotient-only; compare-only target \(g_e^\star=0.0457789\), equivalently \(\Delta_e^{\mathrm{abs},\star}=3.00398633\) & promote the latent candidate \(\widehat C_e^{\mathrm{cand}}\) by closing branch-generator splitting, then emit the refinement-stable uncentered trace lift, then derive the affine-covariant anchor \(A_{\mathrm{ch}}\), then read out \(g_e=e^{A_{\mathrm{ch}}}\) and \(\Delta_e^{\mathrm{abs}}=\log g_{\mathrm{ch,shared}}-A_{\mathrm{ch}}\) \\
Charged leptons & muon & continuation gap & \(n/a\); same centered-shape closure and common-shift no-go as the electron row & same \(\widehat C_e^{\mathrm{cand}} \rightarrow\) promotion \(\rightarrow\) uncentered trace lift \(\rightarrow A_{\mathrm{ch}} \rightarrow g_e\) closure chain \\
Charged leptons & tau lepton & continuation gap & \(n/a\); same centered-shape closure and common-shift no-go as the electron row & same \(\widehat C_e^{\mathrm{cand}} \rightarrow\) promotion \(\rightarrow\) uncentered trace lift \(\rightarrow A_{\mathrm{ch}} \rightarrow g_e\) closure chain \\
Neutrinos & \(\nu_e\) & weighted-cycle continuation & \(n/a\); the weighted-cycle branch emits \(\theta_{12}=34.2259^\circ\), \(\theta_{23}=49.7228^\circ\), \(\theta_{13}=8.68636^\circ\), \(\delta=305.581^\circ\), \(J=-0.02753\), and \(\Delta m_{21}^2/\Delta m_{32}^2=0.03072111\); with compare-only anchor \(\Delta m_{32}^2=2.438\times10^{-3}\,\mathrm{eV}^2\), it gives \(m_1=0.01746\), \(m_2=0.01948\), \(m_3=0.05308~\mathrm{eV}\) & emit the residual bridge scalar \(B_\nu\), after which \(\lambda_\nu\) is induced by exact \(q_{\mathrm{mean}}^{p_\nu}\) factorization; no hidden discrete branch remains on that lane \\
Neutrinos & \(\nu_\mu\) & weighted-cycle continuation & \(n/a\); same weighted-cycle branch as the electron-neutrino row & same residual-bridge frontier \(B_\nu \rightarrow \lambda_\nu\) \\
Neutrinos & \(\nu_\tau\) & weighted-cycle continuation & \(n/a\); same weighted-cycle branch as the electron-neutrino row & same residual-bridge frontier \(B_\nu \rightarrow \lambda_\nu\) \\
...
theorem are also closed, so the remaining attachment gap is one positive residual scalar
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
above the closed normalizer. The current attached stack also factors exactly through
...
\(\mu_e\) family, but that family still sits below \(B_\nu\) rather than replacing it.
