...
\paragraph{Current-premise no-go theorem.}
Let
\[
...
\]
Then the present corpus does not emit
\[
\Theta_{ud}^{\mathrm{mass}},
\qquad
\Theta_{ud}^{\mathrm{phys}},
...
\Theta_{ud}^{\mathrm{abs}}.
\]
On the mass side, \(P\) emits the D12 ray \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\) but not a theorem-grade nonzero value of its modulus \(t_1\). On the mixing side, \(P\) emits the negative selector closure \(\sigma_{ud}=\sigma_{\mathrm{ref}}\), and same-sheet rephasing preserves CKM moduli, so \(P\) emits no sector-attached same-label left-handed lift from \(\sigma_{\mathrm{ref}}\) to the physical CKM shell. On the absolute side, the affine sector-mean split is formula-complete on the selected sheet, but the exact sextet witness remains target-anchored and \texttt{current\_family\_only}, so \(P\) emits no target-free physical-sheet pair \((g_u,g_d)\). Therefore the exact same-family sextet and the D12 internal backread scalar package are continuation-sidecar representatives, not \(P\)-emitted theorem outputs.

\paragraph{Minimal-extension closure theorem.}
The exact minimal extension triple consists of the smallest added data that remove that no-go:
\[
H_{\mathrm{mass}}
\;:\;
...
The quark lane has three theorem-facing open objects on the emitted same-family D12 branch and one separate builder-facing backend burden:
\begin{enumerate}[leftmargin=2em]
\item \(H_{\mathrm{mass}}\): emit the intrinsic light-isospin scalar \(\ell_{ud}:=\log(c_d/c_u)\), equivalently the nonzero one-scalar law \ophid{quark_d12_t1_value_law} on \(D12_{ud}^{\mathrm{mass}}=\mathcal R_{D12}^{ud}\), with \(t_1=\tfrac56\,\ell_{ud}\) and \(\Delta_{ud}^{\mathrm{overlap}}=\tfrac16\,\ell_{ud}=t_1/5\);
\item \(H_{\mathrm{phys}}\): emit the sector-attached same-label left-handed section \(s_{ud}^{\mathrm{phys}}:\mathfrak M_{ud}^{\mathrm{CR,phys}}\to\Sigma_{ud}^{\mathrm{phys}}\), because the emitted local selector is fixed negatively to \(\sigma_{\mathrm{ref}}\) and same-sheet rephasing preserves CKM moduli;
\item \(H_{\mathrm{abs}}\): emit the physical affine anchor \(A_q^{\mathrm{phys}}:\Sigma_{ud}^{\mathrm{phys}}\to\mathbb R\), so the target-free physical-sheet readout \((g_u,g_d)\) follows from the affine mean split and the ordered three-point quadratic readout then emits the sextet; and
\item beneath \(H_{\mathrm{mass}}\), the builder-facing pure-\(B\) payload pair is forced algebraically and is therefore not a fourth theorem object.
\end{enumerate}

The quark headline is narrower: quarks are the only matter family with reported numerical rows, the selected-sheet exact witness reproduces the six running quark reference masses exactly on the same ordered three-point family, and the exact missing theorem package above that sidecar is the minimal extension \((H_{\mathrm{mass}},H_{\mathrm{phys}},H_{\mathrm{abs}})\). Any same-sheet overlap scan, chirality-basis diagnostic, or other orbit improvement that is not sector-attached, left-handed, and same-label remains outside the theorem ledger.
