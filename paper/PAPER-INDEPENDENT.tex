...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Pixel area}: \(a_{\rm cell} \approx 1.63\,\ell_P^2\), the geometric area
...
\item
  \textbf{Screen capacity}: \(\log(\dim\mathcal{H}_{\rm tot}) \sim 10^{122}\), the
...
- N1-N3 (derived, \ensuremath{\S}5.2) \ensuremath{\Longrightarrow} local K{[}I{]} and P = \ensuremath{\int} T\_kk dv
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 4\tabcolsep) * \real{0.5536}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Instrument
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\textbf{Part I (\ensuremath{\S}6.1):} The mathematical reconstruction machinery. Given
...
\textbf{Part II (Derived from MAR, \ensuremath{\S}6.2 onward):} The Selection Axiom MAR
...
\textbar{}\ensuremath{\Phi}\textsuperscript{n}(\ensuremath{\delta}\(\rho_{\mathcal{O}}\))\textbar{} \textasciitilde{} b\textsuperscript{n}\textsuperscript{y}\textbar{}\ensuremath{\delta}\(\rho_{\mathcal{O}}\)\textbar,
...
by U\_ij back to \(\pi_i\)(O). Thus conjugation by \(\Omega{}_i\)jk fixes \(\pi_i\)(O), so
\(\Omega{}_i\)jk commutes with \(\pi_i\)(O). QED.

\textbf{Lemma 6.11 (gauge behavior).} If \textasciitilde\{U\}\_ij = V\_i U\_ij V\_j\ensuremath{\dagger} with V\_i
...
same role: \(\lambda_R\) = \textbar S\textbar{} \ensuremath{-} (1/d\_R) \(\Sigma_{s\in{}S}\) \(\chi_R\)(s).
...
edge charge is the restricted star Q\_v\^{}\{(A)\} = \(\prod_{\ell \in star(v) \cap A}\) X\_\(\ell^{\pm1}\).
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2045}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.5227}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1591}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1136}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedleft
bare g\textsuperscript{2}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
extracted t (mean\ensuremath{\pm}std)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
g\_ent
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
gap
...
\item
  \textbf{S\textsubscript{3}}: Casimir log-ratio (\(\lambda{}_s\)ign/\(\lambda{}_s\)td = 2), precision 0.01\%
\item
...
\item
  ...
  The MS-bar value \^{}\{s\}\textsuperscript{2}\emph{Z = 0.23122 differs from the effective leptonic angle
  s\textsuperscript{2}}\ensuremath{\ell} = 0.23154 and the on-shell value s\textsuperscript{2}\_W = 0.22342. Since we compute from
...
\textbf{Derived couplings at M\_Z.} From \(\alpha{}_e\)m and sin\textsuperscript{2}\(\theta_W\):
...
m\_p \ensuremath{\approx} 4.47 \(\Lambda{}_Q\)CD:
...
**\(\alpha_s\)(M\_Z) = 0.1175, sin\textsuperscript{2}\ensuremath{\theta}\_W(M\_Z) = 0.2311**
\end{quote}
...
\textbf{Numerical outputs.} Using the Peter-Weyl-derived beta shifts and measured \(\alpha{}_e\)m(M\_Z), sin\textsuperscript{2}\(\theta_W\)(M\_Z) to fix \ensuremath{\alpha}\textsubscript{1}, \ensuremath{\alpha}\textsubscript{2}, the edge mechanism predicts:
...
\subsubsection{\texorpdfstring{The Z\textsubscript{6} quotient: edge-sector selection rules and entropy deficit}{The Z quotient: edge-sector selection rules and entropy deficit}}\label{the-z-quotient-edge-sector-selection-rules-and-entropy-deficit}
...
\textbf{Transmutation ansatz.} Assume EWSB is triggered by an edge-sector ordering transition whose scale is set by dimensional transmutation from the UV cell scale, with a one-loop coefficient \(\beta{}_E\)W controlled by the same edge-mode content that produces the MSSM-like beta-function shift:

v = E\_cell \ensuremath{\cdot} exp(\ensuremath{-}2\ensuremath{\pi} / (\(\beta{}_E\)W \ensuremath{\cdot} \(\alpha_U\)))

The edge-mode computation gives \ensuremath{\Delta}b\textsubscript{3} \ensuremath{\approx} 4.00 (Section 6.17). This integer has a structural origin: \(\beta{}_E\)W = N\_c + 1 = 4 is the number of SU(2) doublets per generation (N\_c quark doublets plus one lepton doublet). This is not a fit parameter; it is a topological/anomaly-counting integer already derived in Section 6.9 from the Witten anomaly constraint.
...
\(\beta{}_E\)W = 2\ensuremath{\pi} / (\(\alpha_U\) \ensuremath{\cdot} ln(E\_cell/v\_obs)) \ensuremath{\approx} 4.001

The coefficient demanded by Nature is \(\beta{}_E\)W = 4 to within \textasciitilde0.03\%. This is precisely the integer that appears in the gauge-sector beta-function shift.
...
\subsubsection{\texorpdfstring{Yukawa hierarchy from Z\textsubscript{6} defect suppression}{Yukawa hierarchy from Z defect suppression}}\label{yukawa-hierarchy-from-z-defect-suppression}
...
\ensuremath{\lambda}(M\_U) = 0, \(\beta_{\lambda}\)(M\_U) = 0
...
\(\beta_{\lambda}\) \ensuremath{\propto} \ensuremath{-}6 y\_t\textsuperscript{4} + (3/8)(2g\textsubscript{2}\textsuperscript{4} + (g\textsubscript{2}\textsuperscript{2} + g\textsubscript{1}\textsuperscript{2})\textsuperscript{2})

Setting \(\beta_{\lambda}\)(M\_U) = 0 immediately fixes y\_t(M\_U) in terms of the gauge couplings:
...
\item
  ...
  \end{itemize}
\item
  From \ensuremath{\lambda} = 0, \(\beta_{\lambda}\) = 0: y\_t(M\_U) \ensuremath{\approx} 0.4239
\item
...
2. The refinement-stability constraint \ensuremath{\lambda} = \(\beta_{\lambda}\) = 0 at M\_U
...
\textbf{Chain summary.} The critical surface constraint closes the loop: pixel area \ensuremath{\to} M\_U \ensuremath{\to} (\ensuremath{\lambda} = 0, \(\beta_{\lambda}\) = 0) \ensuremath{\to} y\_t(M\_U) \ensuremath{\to} RG evolution \ensuremath{\to} m\_H \ensuremath{\approx} 125 GeV.
...
This is \(\beta{}_E\)W = 4 to within 0.1\%. The integer 4 = N\_c + 1 (number of SU(2) doublets per generation) emerges from fitting, but also has a structural rationale from the Witten anomaly constraint.
...
\(\beta{}_E\)W = 4 & Structural (N\_c + 1) or fitted; ambiguous status \\
\end{longtable}
}
...
\textbar{}\ensuremath{\langle}\ensuremath{\Delta}K\ensuremath{\rangle}\textbar{} \ensuremath{\leq} 2\textbar A\textbar{} \ensuremath{\cdot} \(\eta^{w/\xi}\)
...
\item
  ...
  the UV and which operator generates v with coefficient \(\beta{}_E\)W = N\_c + 1 = 4.
...
\item
  \textbf{\(\theta{}_Q\)CD} (strong CP problem): See program lemma below.
\item
...
\textbf{Program lemma for \(\theta{}_Q\)CD.} In 3+1D, a \ensuremath{\theta}-term is a topological angle. In the gluing/obstruction language, \ensuremath{\theta} corresponds to a nontrivial 2-group cocycle on triple overlaps:
