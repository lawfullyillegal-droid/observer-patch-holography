...
\subsubsection{1.1 Observers and access model}\label{observers-and-access-model}
...
\subsubsection{1.2 Screen, patches, and algebra net}\label{screen-patches-and-algebra-net}
...
\subsubsection{1.4 Assumptions and external inputs}\label{assumptions-and-external-inputs}
...
\textbf{Clarification (MaxEnt ≠ thermal equilibrium).} MaxEnt here is \textbf{local state
...
\varphi\emph{\{ij\} \varphi}\{jk\} \varphi\emph{\{ki\} = \mathrm{Ad}(z}\{ijk\}),\qquad z\_\{ijk\} \in Z(\mathcal{A}\_\{ijk\}).
...
I(A\_\delta:D\_\delta \mid B\_\delta)\emph{\omega \le \varepsilon(\delta/\ell}\{\mathrm{UV}\}),
...
\textbar{[}A(t), B{]}\textbar{} \le c \textbar A\textbar{} \textbar B\textbar{} \min(\textbar R\_A\textbar, \textbar R\_B\textbar) e\^{}\{-(d - v\_\{\mathrm{LR}\}\textbar t\textbar)/\xi\}
...
\begin{quote}
\sigma\emph{t\^{}\{\omega,C\}(\mathcal{A}(R)) \subseteq \mathcal{A}(R\^{}\{+v}\{\mathrm{mod}\}\textbar t\textbar\})
\quad \text{up to error } \eta(d - v\_\{\mathrm{mod}\}\textbar t\textbar),
\$\$
\end{quote}
...
\begin{quote}
f\_t\^{}C(R) := \lim\emph{\{\ell}\{\mathrm{UV}\} \to 0\} R\^{}\{+v\_\{\mathrm{mod}\}\textbar t\textbar\}.
\$\$
\end{quote}
...
\Phi\emph{\{\ell\to L\}(\omega}\{\ell\}) = \omega\_\{L\},
...
\subsubsection{1.6 Summary: Complete axiom set}\label{summary-complete-axiom-set}
...
\subsection{2. Information-Theoretic Tools}\label{information-theoretic-tools}

\subsubsection{2.1 Strong subadditivity and Markov states}\label{strong-subadditivity-and-markov-states}
...
\rho\emph{\{ABC\} = (\mathrm{id}\emph{A \otimes \mathcal R}\{B\to BC\})(\rho}\{AB\}).
...
\textbar{} \rho\_\{ABC\} - (\mathrm{id}\emph{A \otimes \mathcal R)(\rho}\{AB\}) \textbar\_1
\le 2 \sqrt{\ln 2\, \varepsilon}.

\$\$

\subsubsection{2.3 Collar refinement and sufficient mechanisms}\label{collar-refinement-and-sufficient-mechanisms}

Fix a cap \(C \subset S^2\) with boundary circle. For collar width \(\delta\) define

\$\$

B\_\delta := \{x \in S\^{}2 : \mathrm{dist}(x,\partial C) \le \delta\},
\qquad
A\_\delta := C \setminus B\_\delta,
\qquad
D\_\delta := (S\^{}2 \setminus C) \setminus B\_\delta.
...
A(R) = \mathcal B(\tilde{\mathcal H}\emph{R)\^{}\{G}\{\partial R\}\}.
...
H\_\{B\_\delta\} = \bigoplus\emph{\{\alpha\}
(H}\{b\_L\^{}\{\alpha\}\} \otimes H\_\{b\_R\^{}\{\alpha\}\}),
...
Z(A(B\_\delta)) = \bigoplus\emph{\alpha \mathbb C,\mathbf 1}\alpha,
...
\tilde{\mathcal H}\emph{\{B\_L\} = \bigoplus}\alpha (V\_\alpha \otimes H\_\{b\_L\^{}\alpha\}),\qquad
\tilde{\mathcal H}\emph{\{B\_R\} = \bigoplus}\beta (V\_\beta\^{}* \otimes H\_\{b\_R\^{}\beta\}).
...
\tilde{\mathcal H}\emph{\{B\_L\} \otimes \tilde{\mathcal H}}\{B\_R\}
= \bigoplus\emph{\{\alpha,\beta\}
(V}\alpha \otimes V\_\beta\^{}*) \otimes
(H\_\{b\_L\^{}\alpha\} \otimes H\_\{b\_R\^{}\beta\}).
...
(V\_\alpha \otimes V\_\beta\textsuperscript{*)}\{G\_\Sigma\}
\cong
...
H\_\{B\_\delta\} = \bigoplus\emph{\alpha (H}\{b\_L\^{}\alpha\} \otimes H\_\{b\_R\^{}\alpha\}),
...
A(B\_\delta) = \bigoplus\emph{\alpha
(B(H}\{b\_L\^{}\alpha\}) \otimes B(H\_\{b\_R\^{}\alpha\})),
...
I\_\omega(A\_\delta:D\_\delta \mid B\_\delta) = 0
...
\rho\emph{\{A}\delta B\_\delta D\_\delta\}
= \bigoplus\emph{\alpha p}\alpha \rho\^{}\{(\alpha)\}.
...
\rho\^{}\{(\alpha)\} =
\rho\emph{\{A}\delta b\_L\^{}\alpha\} \otimes \rho\emph{\{b\_R\^{}\alpha D}\delta\}.
...
\qquad
H\_\{\mathrm{eff}\} = \sum\_x \sum\_a \lambda\_a O\_a(x) + \text{(global terms)},
\$\$
\end{quote}
...
I\_\omega(A\_\delta:D\_\delta \mid B\_\delta) \le
c,\lvert\partial C\rvert\emph{\{\mathrm{UV}\},e\^{}\{-\delta/\xi\},
\qquad
\lvert\partial C\rvert}\{\mathrm{UV}\} \sim \frac{\mathrm{length}(\partial C)}{\ell_{\mathrm{UV}}}.
...
\subsubsection{2.6 Concrete UV realization: quantum link models}\label{concrete-uv-realization-quantum-link-models}
...
\tilde{\mathcal{H}}\emph{\{\mathrm{total}\} = \bigotimes}\ell \mathcal{H}\_\ell \otimes \bigotimes\_v \mathcal{H}\_v,
...
\textbar{}\psi\rangle \in \mathcal{H}\_\{\mathrm{phys}\} \quad\Longleftrightarrow\quad U(g\_v)\textbar{}\psi\rangle = \textbar{}\psi\rangle ;;\forall, v,, g\_v \in G\_v.
...
\mathcal{A}(R) = \mathcal{B}(\tilde{\mathcal{H}}\emph{R)\^{}\{G}\{\partial R\}\}.
...
\mathcal{H}\emph{\{B}\delta\} \cong \bigoplus\emph{\alpha (H}\{b\_L\^{}\alpha\} \otimes H\_\{b\_R\^{}\alpha\}),
...
\subsubsection{2.7 Conformal-modular fixed point microphysics (CMFP)}\label{conformal-modular-fixed-point-microphysics-cmfp}
...
\omega \propto \exp\left(-\sum\_a \lambda\_a O\_a\right),
...
f\_t\^{}C(R) := \bigcup\_\{O \in \mathcal{A}(R)\} \mathrm{supp}\left(\sigma\_t\^{}\{\omega,C\}(O)\right).
...
\subsection{3. Overlap Consistency and Gluing}\label{overlap-consistency-and-gluing}

\subsubsection{3.1 Constructive gluing on tree covers}\label{constructive-gluing-on-tree-covers}
...
\textbar{} \rho\^{}\{\ast\}\emph{\{A\_k B\_k C\_k\} - (\mathrm{id}}\{A\_k\} \otimes \mathcal{R}\emph{k)(\rho\^{}\{\ast\}}\{A\_k B\_k\}) \textbar\_1
\le \delta\_k,
...
\delta\_k = 2 \sqrt{\ln 2\, \varepsilon_k}.
...
\textbar{} \hat{\rho} - \rho\^{}\{\ast\} \textbar{}\emph{1 \le \min\left(2, \sum}\{k=2\}\^{}n \delta\_k\right).
...
\subsubsection{3.2 Gauge-as-gluing and loops}\label{gauge-as-gluing-and-loops}
...
H(\gamma) = g\_\{i\_1 i\_2\} g\_\{i\_2 i\_3\} \cdots g\_\{i\_n i\_1\}
...
\subsubsection{3.3 Loop obstruction class (central defect)}\label{loop-obstruction-class-central-defect}

Under Assumption E, define central defects \(z_{ijk}\) by

\$\$

\varphi\emph{\{ij\} \varphi}\{jk\} \varphi\emph{\{ki\} = \mathrm{Ad}(z}\{ijk\}) \quad \text{on } \mathcal{A}\_\{ijk\}.
...
\subsubsection{3.4 Non-central obstruction (2-group cocycle)}\label{non-central-obstruction-2-group-cocycle}
...
g\_\{ij\}: P\_\{ij\} \to G,\qquad h\_\{ijk\}: P\_\{ijk\} \to H,
...
g\_\{ij\} g\_\{jk\} = \partial(h\_\{ijk\}) g\_\{ik\},
...
h\_\{jkl\} h\_\{ijl\} = (g\_\{ij\} \triangleright h\_\{ikl\}) h\_\{ijk\}.
...
\subsection{4. Modular Flow and Lorentz Kinematics}\label{modular-flow-and-lorentz-kinematics}

\subsubsection{4.1 Modular additivity in the Markov collar limit}\label{modular-additivity-in-the-markov-collar-limit}
...
\Delta K := K\_\{ABD\} - K\_\{AB\} - K\_\{BD\} + K\_B.
...
\begin{quote}
\mathcal H\_B = \bigoplus\emph{\{\alpha\} (\mathcal H}\{b\_L\^{}\{\alpha\}\} \otimes \mathcal H\_\{b\_R\^{}\{\alpha\}\}),
\$\$
\end{quote}
...
\begin{quote}
\rho\emph{\{ABD\} = \bigoplus}\{\alpha\} p\_\{\alpha\}
(\rho\emph{\{A b\_L\^{}\{\alpha\}\} \otimes \rho}\{b\_R\^{}\{\alpha\} D\}).
\$\$
\end{quote}
...
\begin{quote}
\log \rho\emph{\{ABD\} = \log \rho}\{A b\_L\^{}\{\alpha\}\} + \log \rho\emph{\{b\_R\^{}\{\alpha\} D\} - \log p}\{\alpha\},
\$\$
\end{quote}
...
\begin{quote}
K\_\{ABD\} = K\_\{AB\} + K\_\{BD\} - K\_B + c\_\alpha,
\$\$
\end{quote}
...
\begin{quote}
\langle \Delta K \rangle\emph{\omega = -I(A:D \mid B)}\omega.
\$\$
\end{quote}
...
= \langle K\_\{AB\} \rangle + \langle K\_\{BD\} \rangle - \langle K\_B \rangle - \langle K\_\{ABD\} \rangle.
\$\$
\end{quote}
...
\begin{quote}
I(A:D\textbar B) = -\langle \Delta K \rangle\_\omega.
\$\$
\end{quote}
...
\subsubsection{\texorpdfstring{4.2 Theorem: \(\mathrm{BW}_{S^2}\) from Markov locality, symmetry, regularity}{4.2 Theorem: \textbackslash mathrm\{BW\}\_\{S\^{}2\} from Markov locality, symmetry, regularity}}\label{theorem-mathrmbw_s2-from-markov-locality-symmetry-regularity}
...
\subsubsection{\texorpdfstring{4.3 Theorem: \(\mathrm{BW}_{S^2}\) implies Lorentz kinematics}{4.3 Theorem: \textbackslash mathrm\{BW\}\_\{S\^{}2\} implies Lorentz kinematics}}\label{theorem-mathrmbw_s2-implies-lorentz-kinematics}
...
\mathrm{Conf}\textsuperscript{+(S}2) \cong \mathrm{PSL}(2,\mathbb C) \cong \mathrm{SO}\^{}+(3,1).

\$\$

\textbf{Proof.} Orientation-preserving conformal maps of \(S^2\) are Möbius
transformations \(\mathrm{PSL}(2,\mathbb{C})\), which is isomorphic to the connected Lorentz
group. QED.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{5. Gravity from Entanglement Equilibrium}\label{gravity-from-entanglement-equilibrium}
...
\delta S\_C = \delta \langle K\_C \rangle.
...
\delta S\_C = 2 \pi \delta \langle B\_C \rangle.

\$\$

\subsubsection{5.2 Null-surface modular bridge}\label{null-surface-modular-bridge}
...
K{[}I\_1 \cup I\_2{]} = K{[}I\_1{]} + K{[}I\_2{]} + K\_\{\partial\} + O(\varepsilon),
...
I\_- = {[}v\_0, v\_1{]}, \quad J = {[}v\_1, v\_2{]} \quad (\text{buffer}), \quad
I\_+ = {[}v\_2, v\_3{]}.
...
\begin{quote}
\mathcal{H}\emph{J \cong \bigoplus}\{\alpha\_1, \alpha\emph{2\}
\left(\mathcal{H}}\{j\_L\^{}\{\alpha\_1, \alpha\emph{2\}\} \otimes
\mathcal{H}}\{j\_R\^{}\{\alpha\_1, \alpha\_2\}\}\right),
\$\$
\end{quote}
...
\begin{quote}
K\_\{I\_- \cup J \cup I\_+\} = K\_\{I\_- \cup J\} + K\_\{J \cup I\_+\} - K\_J
...
\begin{quote}
K{[}I{]} = \int\_I t(v,\Omega),dv + \text{(central term)}.
\$\$
\end{quote}
...
\begin{quote}
\sigma\_t\^{}\{\omega\}(\mathcal{A}(v \textgreater{} v\_1)) \subseteq \mathcal{A}(v \textgreater{} v\_1)
\quad (t \ge 0).
\$\$
\end{quote}
...
K = 2\pi \int v,T\_\{kk\}(v,\Omega),dv + \text{(central term)}.
...
Y\_\{kk\} = Y\_\{00\} + 2 \hat{n}\^{}i Y\_\{0i\} + \hat{n}\^{}i \hat{n}\^{}j Y\_\{ij\}.
...
\langle \hat{n}\emph{i f(\hat{n}) \rangle = \frac{2}{3} T}\{0i\}
\quad \Rightarrow \quad
T\_\{0i\} = \frac{3}{2} \langle \hat{n}\_i f \rangle.
...
\left\langle \left(\hat{n}\emph{i \hat{n}\emph{j - \frac{\delta_{ij}}{3}\right) f(\hat{n}) \right\rangle
= \frac{2}{15} \left(T}\{ij\} - \frac{\delta_{ij}}{3} T}\{kk\}\^{}\{\mathrm{(spatial)}\}\right).
...
\langle f \rangle = T\_\{00\} + \frac{1}{3} T\_\{kk\}\^{}\{\mathrm{(spatial)}\}
...
K{[}I,\Omega{]} = 2\pi \int\emph{I v , T}\{kk\}(v,\Omega) , dv + K\_\partial(I,\Omega) + O(\varepsilon),
...
\subsubsection{5.3 Modular energy as stress-tensor charge (UV CFT)}\label{modular-energy-as-stress-tensor-charge-uv-cft}
...
H\_\{\zeta\} = \int\emph{\{\Sigma\} T}\{ab\} \zeta\^{}b d\Sigma\^{}a,
...
\delta \langle H\_\{\zeta\} \rangle = \frac{4 \pi \ell^4}{15} \delta \langle T\_\{00\} \rangle + O(\ell\^{}5),

\$\$

in the diamond rest frame.

\subsubsection{5.4 Localized generalized entropy from Markov + MaxEnt}\label{localized-generalized-entropy-from-markov-maxent}
...
\rho\emph{C = \bigoplus}\{\alpha\} p\_\{\alpha\}
\left(\rho\_\{\mathrm{bulk},C\}\^{}\{\alpha\} \otimes \frac{\mathbf 1_{\mathrm{edge}}^{\alpha}}{d_{\alpha}}\right).
...
S(\rho\emph{C) = H(p}\{\alpha\}) + \sum\emph{\{\alpha\} p}\{\alpha\} S(\rho\emph{\{\mathrm{bulk},C\}\^{}\{\alpha\}) + \sum}\{\alpha\} p\_\{\alpha\} \log d\_\{\alpha\}.

\$\$

\emph{Convention:} Throughout this paper, ``log'' denotes the natural logarithm (ln),
so entropies are measured in \textbf{nats} (1 nat = 1/ln 2 ≈ 1.443 bits). This is
...
S\_\{\mathrm{bulk}\}(C) := H(p\_\{\alpha\}) + \sum\emph{\{\alpha\} p}\{\alpha\} S(\rho\_\{\mathrm{bulk},C\}\^{}\{\alpha\}),
...
L\_C := \sum\emph{\{\alpha\} (\log d}\{\alpha\}) P\_\{\alpha\}.
...
S\_\{\mathrm{gen}\}(C) := \mathrm{Tr}(\rho L\_C) + S\_\{\mathrm{bulk}\}(C).
...
\mathrm{Tr}(\rho L\_C) \approx N\_\Sigma \cdot \bar\{\ell\}(t),
\qquad
\bar\{\ell\}(t) := \sum\emph{\alpha p}\alpha \log d\_\alpha,
...
A(C) \approx N\_\Sigma \cdot a\_\{\mathrm{cell}\},
...
\subsubsection{5.5 Entanglement equilibrium from MaxEnt}\label{entanglement-equilibrium-from-maxent}
...
\delta S\_\{\mathrm{gen}\}(C) = \delta \langle L\_C \rangle + \delta \langle K\_\{\mathrm{bulk}\} \rangle.

\$\$

\subsubsection{5.6 Einstein equation from cap equilibrium}\label{einstein-equation-from-cap-equilibrium}
...
\delta A\textbar{}\emph{\{V,\lambda\} = -\frac{\Omega_{d-2} \ell^d}{d^2 - 1} (G}\{00\} + \lambda g\_\{00\}).
...
G\_\{00\} + \Lambda g\_\{00\} = 8 \pi G \langle T\_\{00\} \rangle,

\$\$

in the diamond rest frame, with \(\Lambda\) fixed by the reference curvature.

\subsubsection{5.7 Overlaps supply all timelike directions}\label{overlaps-supply-all-timelike-directions}
...
G\_\{ab\} + \Lambda g\_\{ab\} = 8 \pi G \langle T\_\{ab\} \rangle.

\$\$

\subsubsection{5.8 Non-tunable numerical constants}\label{non-tunable-numerical-constants}
...
\int\emph{\{B\^{}\{d-1\}}\ell\} \frac{\ell^2 - r^2}{2\ell} , d\^{}\{d-1\}x
...
\subsubsection{5.9 Quantitative Markov error and controlled corrections}\label{quantitative-markov-error-and-controlled-corrections}
...
\Delta K\_\delta := K\_\{ABD\} - K\_\{AB\} - K\_\{BD\} + K\_B,
...
\langle \Delta K\_\delta \rangle\emph{\omega = -I(A:D\textbar B)}\omega.
...
\left\textbar{} \langle \Delta K\_\delta \rangle\emph{\omega \right\textbar{}
\le c , \textbar{}\partial C\textbar{}}\{\mathrm{UV}\} , e\^{}\{-\delta/\xi\}.
...
K\_C = 2\pi B\_C + K\_C\^{}\{\mathrm{(anom)}\},
...
G\_\{00\} + \Lambda g\_\{00\} = 8\pi G \left( \langle T\_\{00\} \rangle
+ \langle T\_\{00\}\^{}\{\mathrm{anom}\} \rangle \right),
...
\langle T\_\{00\}\^{}\{\mathrm{anom}\} \rangle
...
\left\textbar{} \langle T\_\{00\}\^{}\{\mathrm{anom}\} \rangle \right\textbar{}
\lesssim \frac{15}{8\pi^2} \cdot \frac{1}{\ell^4} \cdot
c , \textbar{}\partial C\textbar\_\{\mathrm{UV}\} , e\^{}\{-\delta/\xi\}.
...
\subsubsection{5.10 Focusing/QNEC internalization via relative entropy}\label{focusingqnec-internalization-via-relative-entropy}
...
- N1-N3 (derived, §5.2) ⟹ local K{[}I{]} and P = ∫ T\_kk dv
...
S(\rho\emph{\{AB\} \textbar{} \sigma}\{AB\}) \ge S(\rho\_A \textbar{} \sigma\_A).
...
\frac{d^2}{d\lambda^2} S(\rho\emph{\lambda \textbar{} \sigma}\lambda) \ge 0.
...
\begin{quote}
\frac{d^2 S_{\mathrm{bulk}}}{d\lambda^2} \le 2\pi \langle T\_\{kk\}(\lambda) \rangle,
\$\$
\end{quote}
...
\begin{quote}
S(\rho\emph{\{R(\lambda)\} \textbar{} \omega}\{R(\lambda)\}) \le S(\rho\emph{\{R(\lambda')\} \textbar{} \omega}\{R(\lambda')\}).
\$\$
\end{quote}
...
\begin{quote}
S\_\{\mathrm{gen}\} = \mathrm{Tr}(\rho L\_C) + S\_\{\mathrm{bulk}\}.
\$\$
\end{quote}
...
\subsubsection{5.11 Discrete horizon area spectrum and Hawking emission (speculative)}\label{discrete-horizon-area-spectrum-and-hawking-emission-speculative}
...
A\_\alpha = 4G \log d\_\alpha = 4\ell\emph{p\^{}2 \ln d}\alpha,
...
\Delta E\_k = k\_B T\_H \ln k, \qquad
\Delta f\_k = \frac{c^3}{16\pi^2 G M} \ln k.
...
\Delta A = 4\ell\_p\^{}2 \ln 3 \approx 4.39 , \ell\_p\^{}2
...
\hbar(\omega - m\Omega\_H) = k\_B T\_H \ln k, \qquad k \in \{2, 3, 4, \ldots\}
...
\Omega\_H(M,\chi) = \frac{c^3}{2GM} \cdot \frac{\chi}{1+\sqrt{1-\chi^2}}.
...
f\_\{k,m\}(M,\chi) = \frac{m \, \Omega_H(M,\chi)}{2\pi} + \frac{c^3}{16\pi^2 GM} , g(\chi) , \ln k
...
W\^{}\{\mathrm{net}\}\emph{\{k,\ell m\} = \Gamma\^{}\{\mathrm{GR}\}}\{\ell m\}(\omega\_\{k,m\}) \cdot \Delta\omega\_k \cdot \frac{k-1}{k}
...
x := \frac{GM}{c^3 g(\chi)}(\omega - m\Omega\_H).
...
x\_k = \frac{\ln k}{8\pi} \qquad (k = 2, 3, 4, \ldots)
...
\subsubsection{5.12 Classical mechanics from emergent GR}\label{classical-mechanics-from-emergent-gr}
...
\nabla\_a(\rho u\^{}a u\^{}b) = 0
\quad \Rightarrow \quad
u\^{}b \nabla\_a(\rho u\^{}a) + \rho , u\^{}a \nabla\_a u\^{}b = 0.
...
g\_\{00\} \approx -(1 + 2\Phi/c\^{}2), \qquad
g\_\{0i\} \approx 0, \qquad
g\_\{ij\} \approx \delta\_\{ij\}(1 - 2\Phi/c\^{}2),
...
\subsubsection{5.13 Precision gravity predictions and experimental bounds}\label{precision-gravity-predictions-and-experimental-bounds}
...
-3 \times 10\^{}\{-15\} \textless{} \frac{c_{\mathrm{GW}} - c}{c} \textless{} +7 \times 10\^{}\{-16\}
...
m\_g \le 1.76 \times 10\^{}\{-23\} \text{ eV}/c\^{}2 \quad (90\% \text{ credibility}).
...
-4 \times 10\^{}\{-6\} \textless{} \delta\hat{p}\_\{-2\} \textless{} 2 \times 10\^{}\{-5\}
\quad (90\% \text{ credibility}).
...
\subsubsection{5.14 Theory-side error propagation from Markov bounds}\label{theory-side-error-propagation-from-markov-bounds}
...
I(A\_k : C\_k \mid B\_k) \le \varepsilon\_k,
...
\delta\_k = 2\sqrt{\ln 2 \cdot \varepsilon_k}.
...
\textbar{}\langle O \rangle\emph{\rho - \langle O \rangle}\sigma\textbar{}
\le \textbar O\textbar\_\infty \textbar{}\rho - \sigma\textbar{}\emph{1 = 2 \textbar O\textbar{}}\infty D(\rho, \sigma),
...
I\_\omega(A\_\delta : D\_\delta \mid B\_\delta)
\le c \cdot \textbar{}\partial C\textbar\_\{\mathrm{UV}\} \cdot e\^{}\{-\delta/\xi\}.
...
\delta\_\{\mathrm{step}\} \lesssim 2\sqrt{\ln 2 \cdot c \cdot |\partial C|_{\mathrm{UV}}}
\cdot e\^{}\{-\delta/(2\xi)\}.
...
\subsubsection{5.15 Dark matter as modular anomaly (program-level)}\label{dark-matter-as-modular-anomaly-program-level}
...
\nabla\^{}2 \Phi = 4\pi G (\rho\emph{b + \rho}\{\mathrm{anom}\}),
...
g\_\{\mathrm{obs}\} \approx g\_b + \sqrt{a_0 \cdot g_b},
\quad
g\_\{\mathrm{DM}\} := g\_\{\mathrm{obs}\} - g\_b \approx \sqrt{a_0 \cdot g_b}.
...
V\^{}4 \approx G \cdot M\_b \cdot a\_0\^{}\{\mathrm{(OPH)}\}
...
g\_\{\mathrm{DM}\}(r) = \frac{\sqrt{G M_b \, a_0^{\mathrm{(OPH)}}}}{r}
\quad \Rightarrow \quad
M\_\{\mathrm{DM}\}(r) \propto r
...
\Sigma\emph{0\^{}\{\mathrm{(OPH)}\} = \frac{a_0^{\mathrm{(OPH)}}}{2\pi G}
\approx 0.25 \text{ kg/m}\^{}2 \approx 120 , M}\odot/\text{pc}\^{}2.
...
\subsubsection{5.16 De Sitter holography: static patch vs boundary-at-infinity}\label{de-sitter-holography-static-patch-vs-boundary-at-infinity}
...
\subsection{6. Standard Model from Gluing Consistency}\label{standard-model-from-gluing-consistency}
...
\subsubsection{6.1 Edge sector category and gauge group reconstruction}\label{edge-sector-category-and-gauge-group-reconstruction}
...
\subsubsection{6.2 Selecting the SM factors (derived from MAR)}\label{selecting-the-sm-factors-derived-from-mar}
...
G \cong G\_1 \times G\_2,
\qquad
\mathsf{Sect}\_i \simeq \mathrm{Rep}(G\_i).
...
G\_\{\mathrm{phys}\} = \frac{\mathrm{SU}(3)\times \mathrm{SU}(2)\times \mathrm{U}(1)}{\mathbb Z_6}.
...
G\_\{\mathrm{phys}\} = \frac{SU(3) \times SU(2) \times U(1)}{Z_6}.

\$\$

The full proof is in GAUGE\_GROUP\_DERIVATION.md.

\subsubsection{6.3 Refinement stability and unprotected relevant operators}\label{refinement-stability-and-unprotected-relevant-operators}
...
in the emergent EFT sense (Δ\textless4 in 3+1D), allowed by symmetry,
...
Φ\_\{ℓ→L\} around the MaxEnt state ω\_ℓ. Constraint-
preserving perturbations δρ evolve as
δρ' = Φ\_\{ℓ→L\}(δρ). A relevant operator corresponds
to an unstable eigen-direction δρ\_𝒪 with
\textbar Φⁿ(δρ\_𝒪)\textbar{} \textasciitilde{} bⁿʸ\textbar δρ\_𝒪\textbar,
y\textgreater0, under repeated coarse-graining. If 𝒪 is not fixed by
𝒞 or symmetry, any small UV mismatch produces a nonzero
component along δρ\_𝒪, which grows under refinement,
...
\subsubsection{6.4 Generation number from CP violation and refinement stability (derived from MAR)}\label{generation-number-from-cp-violation-and-refinement-stability-derived-from-mar}
...
phases in an N\_g × N\_g CKM matrix is:
...
b\_\{1,\mathrm{SU}(2)\} = \frac{1}{3}\left[22 - N_g(N_c + 1)\right].
...
4 N\_g \textless{} 22 \quad \Rightarrow \quad N\_g \le 5.
...
\subsubsection{6.5 Hilbert-space formulation of gluing data}\label{hilbert-space-formulation-of-gluing-data}

Let \{P\_i\} be a good cover of the screen. For each patch, fix a representation

\$\$

\pi\_i: \mathcal{A}\_i \to \mathcal B(\mathcal H\_i).
...
U\_\{ij\}: \mathcal H\_j \to \mathcal H\_i
...
\pi\emph{i(O) = U}\{ij\} \pi\emph{j(O) U}\{ij\}\^{}\dagger.
...
\Omega\emph{\{ijk\} := U}\{ij\} U\_\{jk\} U\_\{ki\}.
...
\Omega\_\{ijk\} \pi\_i(O) = \pi\emph{i(O) \Omega}\{ijk\}.

\$\$

\textbf{Proof.} Conjugation by U\_ki sends π\_i(O) to π\_k(O), by U\_jk to π\_j(O),
by U\_ij back to π\_i(O). Thus conjugation by Ω\_ijk fixes π\_i(O), so
Ω\_ijk commutes with π\_i(O). QED.

\textbf{Lemma 6.11 (gauge behavior).} If Ũ\_ij = V\_i U\_ij V\_j† with V\_i
acting trivially on overlap observables, then

\$\$

\tilde{\Omega}\emph{\{ijk\} = V\_i \Omega}\{ijk\} V\_i\^{}\dagger.

\$\$

In particular, if Ω\_ijk is central, its class is gauge invariant. QED.

\subsubsection{6.6 Loop obstruction class (central defect)}\label{loop-obstruction-class-central-defect-1}

Assume the defect is central and write

\$\$

\varphi\emph{\{ij\} := \mathrm{Ad}(U}\{ij\}) \quad \text{on } \mathcal{A}\_\{ij\}.
...
\varphi\emph{\{ij\} \varphi}\{jk\} \varphi\emph{\{ki\} = \mathrm{Ad}(z}\{ijk\}).
...
z\_\{jkl\} z\_\{ikl\}\^{}\{-1\} z\_\{ijl\} z\_\{ijk\}\^{}\{-1\} = 1.
...
φ\_ij φ\_jk φ\_kl φ\_li on a quadruple overlap to
...
\subsubsection{6.7 EFT reduction to anomaly cancellation}\label{eft-reduction-to-anomaly-cancellation}
...
\subsubsection{6.8 Hypercharge from anomaly freedom and Yukawas}\label{hypercharge-from-anomaly-freedom-and-yukawas}

\textbf{Theorem 6.13 (Hypercharge from anomaly freedom and Yukawas).} Assume gauge
group SU(N\_c) × SU(2) × U(1)\_Y and one generation of left-handed Weyl
...
Y\_L = -N\_c Y\_Q,\quad
Y\_H = N\_c Y\_Q,\quad
Y\_u = -(N\_c + 1) Y\_Q,\quad
Y\_d = (N\_c - 1) Y\_Q,\quad
Y\_e = 2 N\_c Y\_Q.

\$\$

With these relations, SU(N\_c)²U(1) and U(1)³ anomalies vanish
automatically. Fixing the normalization by Q = T₃ + Y and Q(ν\_L)=0 gives
...
Y\_Q = \frac{1}{6},\quad Y\_L = -\frac{1}{2},\quad Y\_e = 1,\quad
Y\_u = -\frac{2}{3},\quad Y\_d = \frac{1}{3},\quad Y\_H = \frac{1}{2}.
...
\subsubsection{6.9 Witten anomaly and the number of colors}\label{witten-anomaly-and-the-number-of-colors}

\textbf{Theorem 6.14 (The number of colors is N\_c = 3).}
Under the gauge structure SU(N\_c) × SU(2)\_L × U(1)\_Y with one
...
N\_c + 1 \equiv 0 \pmod{2} \quad \Rightarrow \quad N\_c \text{ is odd}.

\$\$

The Witten constraint alone allows N\_c ∈ \{1, 3, 5, 7, \ldots\}. N\_c = 1
...
\subsubsection{6.10 Bond-dimension gatekeeping}\label{bond-dimension-gatekeeping}
...
\subsubsection{6.11 Inevitability of photon and graviton}\label{inevitability-of-photon-and-graviton}
...
\tightlist
\item
  Theorem 4.2 (BW\_\{S²\}) shows that under collar Markov locality, MaxEnt
...
\item
  ...
  δS\_gen = 0 implies the semiclassical Einstein equations in the EFT regime.
\item
...
\subsubsection{6.12 Mass predictions from symmetry}\label{mass-predictions-from-symmetry}
...
\textbf{Experimental status}: PDG 2025 lists m\_g ≤ 1.76 × 10⁻²³
eV/c² (90\% CL) from gravitational wave dispersion analysis. The GW speed
bound from GW170817 constrains (c\_GW − c)/c to \textasciitilde10⁻¹⁵. The
...
G\_\{\mathrm{phys}\} = \frac{\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)}{Z_6},
...
\subsubsection{6.13 Coupling extraction from edge-sector probabilities}\label{coupling-extraction-from-edge-sector-probabilities}

The edge-center completion (Theorem 2.3) yields sector probabilities p\_α
...
sectors are labeled by charge q ∈ \{0, 1, \ldots, n−1\}. The correct
``Casimir'' eigenvalue is the Laplacian eigenvalue of the boundary random walk:

\$\$

\lambda\_q = 4 \sin\^{}2\left(\frac{\pi q}{n}\right).

\$\$

Note: only in the limit n → ∞ and q ≪ n does λ\_q ≈
...
g\_\{\mathrm{ent}\}\^{}2(\mu) = \frac{t(\mu)}{2\pi}.
...
Q\_v\^{}\{(A)\} = \prod\emph{\{\ell \in \mathrm{star}(v) \cap A\} X}\ell\^{}\{\pm 1\},
...
P\_\{v,q\} = \frac{1}{n} \sum\_\{m=0\}\^{}\{n-1\} \omega\^{}\{-mq\} \left(Q\_v\textsuperscript{\{(A)\}\right)}m,
...
p\_j \propto d\_j , e\^{}\{-t(\mu) C\_2(j)\},
...
p\_R = \frac{d_R \, e^{-t \lambda_R}}{\sum_{R'} d_{R'} \, e^{-t \lambda_{R'}}}

\$\$

where λ\_R is the Laplacian eigenvalue on the R-isotypic component
...
L\^{}2(G) \cong \bigoplus\_R V\_R \otimes V\_R\^{}*
...
ℋ\_\{edge\}\^{}\{phys\} = ⊕\_R W\_R where W\_R
contains states with flux in representation R.

\emph{Step 3 (Natural Hamiltonian).} From LG, the MaxEnt generator restricted to
edge modes takes the form H\_\{edge\} = Σ\_R h\_R P\_R where P\_R is
the projector onto the R-sector. The key claim is that h\_R = λ\_R.
...
same role: λ\_R = \textbar S\textbar{} − (1/d\_R) Σ\_\{s∈S\} χ\_R(s).

\emph{Step 4 (MaxEnt selection).} MaxEnt (Assumption B) selects the Gibbs state:

\$\$

\rho\emph{\{\mathrm{edge}\} = \frac{1}{Z} e\^{}\{-t H}\{\mathrm{edge}\}\} = \frac{1}{Z}
\sum\_R e\^{}\{-t \lambda\_R\} P\_R.

\$\$

\emph{Step 5 (Sector probabilities).} The probability of sector R is
p\_R = Tr(ρ\_\{edge\} P\_R). The effective dimension for
entanglement is d\_R (not d\_R²) because we trace over one side of the
cut. This gives:

\$\$

p\_R = \frac{d_R \, e^{-t \lambda_R}}{Z}.

\$\$

QED.

\textbf{Why d\_R and not d\_R²?} The full edge space has dimension d\_R² in
sector R (from V\_R ⊗ V\_R\emph{), but entanglement entropy measures
...
H = \frac{g^2}{2} \Delta\_G, \qquad
\Delta\_G \chi\_R = -C\_2(R) \chi\_R
\quad \Rightarrow \quad
E\_R = \frac{g^2}{2} C\_2(R).

\$\$

Euclidean evolution for ``time'' A (the area of a cylinder in 2D YM) gives
weight(R) ∝ exp(−A E\_R) = exp(−g² A C₂(R)/2). Comparing with the
heat-kernel expansion K\_t(U) = Σ\_R d\_R χ\_R(U) e\^{}\{−t C₂(R)\} yields the
exact identification:

\$\$

t\_\mathrm{phys} = \frac{g^2 A}{2} \quad \text{(in 2D YM, no ambiguity).}
...
\subsubsection{6.14 Numerical validation of the heat-kernel law}\label{numerical-validation-of-the-heat-kernel-law}
...
\textbf{Model.} A 2×2 periodic lattice gauge theory (8 links) with
Z\_n link Hilbert spaces and Hamiltonian:

\$\$

H = -K \sum\emph{p \mathrm{Re}(B\_p) - h \sum}\ell \mathrm{Re}(X\_\ell) - \Gamma \sum\_v \mathrm{Re}(A\_v),

\$\$

where X\_ℓ is the Z\_n shift on link ℓ, B\_p is the oriented
...
K = 1 and Γ = 5, the ground state satisfies ⟨A\_v⟩ = 1
...
edge charge is the restricted star Q\_v\^{}\{(A)\} = ∏\_\{ℓ ∈ star(v) ∩ A\} X\_ℓ\^{}\{±1\}.
...
\textbf{Results for Z₃ (overconstrained test).} With
λ₁ = λ₂ = 4sin²(π/3) = 3:
...
agreement to \textasciitilde10⁻¹⁴ (machine precision) confirms that the edge
...
- 2 links (one vertex's outgoing links): g\_ent ≈ 0.453
...
model (λ\_q ∝ q) would predict ratio 2, while a quadratic model
...
\textbf{Results for S₃ (first nonabelian test).} The abelian tests above use
...
P\_\{v,R\} = \frac{d_R}{|G|} \sum\_\{h \in G\} \chi\_R(h\^{}\{-1\}) Q\_v\^{}\{(A)\}(h),
...
\lambda\emph{\{\mathrm{triv}\} = 0, \qquad \lambda}\{\mathrm{sign}\} = 6, \qquad
\lambda\_\{\mathrm{std}\} = 3.
...
by the character states \{\textbar χ\_R⟩\}. In this basis, the edge-sector
probabilities are exactly p\_R = \textbar c\_R\textbar² where \textbar ψ₀⟩ = Σ\_R c\_R
\textbar χ\_R⟩. This is not an approximation; it is an exact identity for the
one-plaquette gauge-invariant sector.

The heat-kernel ansatz predicts p\_R ∝ d\_R exp(−t λ\_R). Extracting
t independently from the sign and standard irreps provides an overconstrained
test: the ratio λ\_sign/λ\_std = 6/3 = 2 is a
...
The ``Δt/t'' column shows the fractional difference (t\_sign −
t\_std) / t̄. The ``log-ratio'' column shows
log(p\_sign/p₀) / log(p\_std/(2 p₀)), which should
equal λ\_sign/λ\_std = 2 if the heat-kernel
form holds exactly.
...
\Delta\_R \equiv \ln\left(\frac{p_0}{d_0}\right) - \ln\left(\frac{p_R}{d_R}\right)
= t , C\_2(R).
...
C₂(p,q) = (p² + q² + pq + 3p + 3q)/3. Using the fundamental \textbf{3} = (1,0)
...
(finite truncated irrep basis, n\_max = 12, κ = 2) has been used to
...
suggests fitting A\_eff(g²) = A₀ + a · g². A weighted linear
fit gives:
...
microphysics, the conversion g² = 2t/A\_eff fixes the gauge
...
\subsubsection{6.15 Particle mass extraction from spectroscopy}\label{particle-mass-extraction-from-spectroscopy}
...
where \textbar n⟩ is the lowest excited eigenstate with ⟨n\textbar O\textbar0⟩ ≠ 0
and E₀ is the ground state energy.

\textbf{Plaquette channel.} For the Z₃ model, using
O = Σ\_p Re(B\_p), the extracted masses m\_plaq are
...
scheme. The ratio m\_plaq / g\_ent² is a pure number
that can be compared across different bare couplings to check scaling.

\subsubsection{6.16 Composite masses and the path to predictions}\label{composite-masses-and-the-path-to-predictions}
...
m\_p = \Lambda\emph{\{\mathrm{QCD}\} \cdot F\left(\frac{m_u}{\Lambda_{\mathrm{QCD}}},
\frac{m_d}{\Lambda_{\mathrm{QCD}}}, \frac{m_s}{\Lambda_{\mathrm{QCD}}}, \ldots;
\alpha}\{\mathrm{em}\}\right),
...
A 0.1\% uncertainty in α\_s becomes approximately 0.7\% uncertainty in the
hadronic mass scale.

\subsubsection{6.17 Gauge unification and spectrum constraints}\label{gauge-unification-and-spectrum-constraints}
...
α\_s consistency is a \emph{consistency check} with known physics, not a
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Canonical GUT normalization}: α₁ ≡ (5/3)α\_Y, the
...
\item
  ...
  \emph{Note on sin²θ\_W schemes:} PDG lists multiple definitions with different values.
...
\alpha\_2 = \frac{\alpha_{\mathrm{em}}}{\sin^2\theta_W}, \quad
\alpha\_Y = \frac{\alpha_{\mathrm{em}}}{1 - \sin^2\theta_W}, \quad
\alpha\_1 = \frac{5}{3}\alpha\_Y.
...
\textbf{Analytic prediction formula.} Define A\_i = α\_i⁻¹(M\_Z) and
L = ln(M\_U/M\_Z). The RG equations give A\_i = A\_U + (b\_i/2π)L. Taking
differences to eliminate A\_U:

\$\$

L = \frac{2\pi}{b_1 - b_2}(A\_1 - A\_2).
...
A\_3\^{}\mathrm{pred} = \frac{b_3 - b_2}{b_1 - b_2} A\_1 + \frac{b_1 - b_3}{b_1 - b_2} A\_2
...
\alpha\emph{s(M\_Z)\textbar{}}\mathrm{SM,unif} = 0.07107 \pm 0.00005
...
α\_s(M\_Z) = 0.1177 ± 0.0009. The SM-only prediction misses by
Δα\_s ≈ 0.047, a \textasciitilde52σ discrepancy, far too large to be rescued
...
\alpha\emph{s(M\_Z)\textbar{}}\mathrm{MSSM,unif} = 0.11658 \pm 0.00015
...
\textbf{Comparison to measurement}: The PDG 2025 EW-fit value is
α\_s(M\_Z) = 0.1177 ± 0.0009. The mismatch is
Δα\_s ≈ −0.0011, about 1.2σ. This is within the
...
are active all the way down to M\_Z. If the Δb only turns on above
...
α\_s⁻¹ by \textasciitilde0.6 to reach the experimental value.

\textbf{Inverted problem: derive M\_S from measured couplings.} With three measured
couplings (A₁, A₂, A₃) and three unknowns (M\_S, M\_U, α\_U), the
...
The uncertainty is dominated by the experimental error on α\_s(M\_Z) = 0.1177 ± 0.0009.
The central value is sensitive to the precise α\_s input: α\_s = 0.1175 gives
M\_S ≈ 67 GeV, while α\_s = 0.1166 (the MSSM prediction) gives M\_S ≈ 91 GeV.
...
\tau\_p\^{}\{\mathrm{gauge}\} = \infty \quad \text{(no gauge-mediated proton decay)}
...
\textbf{Proposition (Factorization ↔ additive boundary Laplacian).} Suppose the
edge Hamiltonian governing boundary sector weights takes the form:

\$\$

H\_\{\partial\} = H\^{}\{(1)\}\emph{\{\partial\} + H\^{}\{(2)\}}\{\partial\} + H\^{}\{(3)\}\_\{\partial\},
...
p(R\_1, R\_2, R\_3) \propto \prod\emph{\{i=1\}\^{}3 d}\{R\_i\} e\^{}\{-t\_i C\_2(R\_i)\}.
...
\textbf{Experimental status.} Minimal SU(5) GUTs predict τ\_p \textasciitilde{} 10³¹
years; Super-Kamiokande has pushed limits to τ\_p \textgreater{} 10³⁴ years,
...
unification} (MSSM-like α\_s consistency) with \textbf{proton stability}
...
\textbf{Chain summary}: Edge-sector probabilities → gauge couplings at UV scale
→ one-loop RG → consistency check for α\_s(M\_Z) → spectrum
...
\textbf{(A) In Planck units (a\_cell/ℓ\_p²).} Once the dimensionless entropy density ℓ̄ is fixed, the dimensionless pixel area is a\_cell/ℓ\_p² = 4ℓ̄. This ratio is \textbf{independent of the experimental uncertainty in G}, because ℓ\_p² ∝ G cancels. The limiting precision is whatever uncertainty remains in ℓ̄, i.e., in the inputs used to determine t(μ) (currently gauge couplings). Since we feed in SM couplings to get ℓ̄, precision is limited by those inputs, not by G.

\textbf{(B) In SI units (a\_cell {[}m²{]}).} If ℓ̄ were known exactly, then a\_cell in SI units would inherit the uncertainty of G:

\$\$

\ell\_p = \sqrt{\frac{\hbar G}{c^3}} \quad \Rightarrow \quad
...
\textbf{Reverse-engineering α\_s from the pixel-area scale.} The pixel-area relation
(Section 5.4) provides an independent route to α\_s(M\_Z). The cell area in
Planck units is:

\$\$

\frac{a_{\mathrm{cell}}}{\ell_p^2} = 4 \bar\{\ell\}\emph{\{\mathrm{tot}\}(t\_2, t\_3),
\quad
\bar\{\ell\}}\{\mathrm{tot}\} = \bar\{\ell\}\emph{\{\mathrm{SU}(2)\}(t\_2) + \bar\{\ell\}}\{\mathrm{SU}(3)\}(t\_3),
...
\textbf{RG mechanism from Markov collar structure.} The running of gauge couplings is not an extra assumption but a structural consequence of the Markov collar plus symmetry. Consider a nested family of caps C(δ) and the operation ``thicken the collar by Δ'':
...
\mathsf{K}\emph{\{t\_1\} \star \mathsf{K}}\{t\_2\} = \mathsf{K}\_\{t\_1 + t\_2\},
...
Using the measured \(a_{\mathrm{cell}}/\ell_p^2 = 1.63094\) and α₂(M\_Z) ≈ 0.0338
...
This is consistent with the PDG EW-fit value α\_s(M\_Z) = 0.1177 ± 0.0009 to within
\textasciitilde5 × 10⁻⁵. The agreement is striking but \textbf{conditional}: it becomes a
genuine prediction only if \(a_{\mathrm{cell}}/\ell_p^2\) can be fixed
independently (from the gravity side) rather than computed from α\_s.

\textbf{Proton mass estimate.} Using standard 4-loop \(\overline{\mathrm{MS}}\) running
with n\_f = 5 at M\_Z, the above α\_s corresponds to
Λ\_\{\overline{\mathrm{MS}}\}\^{}\{(5)\} ≈ 0.208 GeV. With the lattice-motivated ansatz
m\_p ≈ 4.47 Λ\_QCD:

\$\$

m\_p\^{}\{\mathrm{est}\} \approx 4.47 \times 0.208 \simeq 0.93 \text{ GeV},
...
\textbf{Simultaneous prediction of α\_s and sin²θ\_W.} The pixel-area constraint can
...
- sin²θ\_W(M\_Z): predicted 0.2311 vs PDG ŝ²\_Z = 0.23122 ± 0.00006 (MS-bar), difference \textasciitilde1×10⁻⁴ (\textasciitilde2σ)

\textbf{Important caveat on sin²θ\_W.} In \emph{percentage} terms, 0.05\% looks impressive. But
the MS-bar experimental uncertainty is ±0.00006, so the difference \textasciitilde1×10⁻⁴ is
\textasciitilde2σ in experimental units.

This residual discrepancy is not a calculation bug. For MSSM one-loop coefficients,
unification implies a tight relation between α\_s and sin²θ\_W once α̂ is fixed:

\$\$

\sin\^{}2\hat{\theta}\_W(M\_Z) = \frac{1}{5} + \frac{7}{15}\frac{\hat{\alpha}(M_Z)}{\alpha_s(M_Z)}.

\$\$

Once the pipeline outputs α\_s ≈ 0.1175, the weak mixing angle is essentially
locked near 0.231. The \textasciitilde2σ residual reflects missing theoretical corrections,
not a flaw in the core calculation.

The required correction is small: shifting sin²θ\_W by \textasciitilde1×10⁻⁴ corresponds to
only \textasciitilde0.04\% change in α₂(M\_Z). Effects at this level include:
...
omits. The claim is therefore: the framework produces sin²θ\_W within \textasciitilde2σ
...
framework cannot claim precision better than \textasciitilde0.1\% on sin²θ\_W. The \textasciitilde2σ
...
sin²θ\_W(M\_Z) as inputs. The pixel-area constraint eliminates sin²θ\_W as an
input, predicting it instead.

\textbf{SM falsification.} Repeating with SM-only beta coefficients
(b₁, b₂, b₃) = (41/10, −19/6, −7) gives α\_s ≈ 0.096 and sin²θ\_W ≈ 0.216,
...
L\^{}2(G) \simeq \bigoplus\_R V\_R \otimes V\_R\^{}*
...
\textbf{Representation bookkeeping.} Complex representations R and their conjugates R̄ are counted separately in the sum. For SU(3), the fundamental \textbf{3} and antifundamental \textbf{3̄} both contribute with d = 3, C₂ = 4/3, and T = 1/2. Real representations (like the adjoint \textbf{8}) appear once. This is standard QFT bookkeeping: each chiral fermion species contributes independently to vacuum polarization.
...
This achieves MSSM-like beta shifts without inserting MSSM by hand. The \textasciitilde5\% tension in Δb₂ may be resolved by two-loop corrections, threshold effects, or refinements to the U(1) sector weighting.

\textbf{What makes this non-trivial.} The key test is not matching individual Δb values (which can be achieved by adjusting an overall normalization), but the \textbf{ratio} Δb₃/Δb₂. The MSSM requires Δb₃/Δb₂ = 4.00/4.17 = 0.959. The Peter-Weyl calculation gives 3.97/4.38 = 0.906, about 6\% low. This ratio is fixed by the heat-kernel distribution and representation theory, with no free parameters to adjust. Getting within 6\% of a non-trivial ratio like 0.96 from first principles is significant, though the remaining discrepancy indicates the mechanism is not yet complete.
...
\Delta b\_a(t) = 4\pi \langle T\_a \rangle\emph{\{p(t)\}, \qquad
\langle T\_a \rangle}\{p(t)\} := \sum\_R p\_R(t) , T\_a(R).
...
b\^{}\{\rm UV\} = b\^{}\{\rm SM\} + \Delta b\_\{\rm edge\} \approx (6.59, 1.22, -3.03).
...
M\_S \approx 100 \text{ TeV}, \quad M\_U \approx 6.5 \times 10\^{}\{15\} \text{ GeV}, \quad \alpha\_U\^{}\{-1\} \approx 28.3
...
\subsubsection{6.18 The Z₆ quotient: edge-sector selection rules and entropy deficit}\label{the-zux2086-quotient-edge-sector-selection-rules-and-entropy-deficit}
...
- u\^{}c: (3̄, 1, Y=−2/3) =\textgreater{} (τ=2, j=0, n=−4) ✓
- d\^{}c: (3̄, 1, Y=1/3) =\textgreater{} (τ=2, j=0, n=2) ✓
- e\^{}c: (1, 1, Y=1) =\textgreater{} (τ=0, j=0, n=6) ✓

\textbf{Heat-kernel slopes at M\_Z.} The general relation is t = g²A\_eff/2 where
A\_eff is the effective ``Euclidean thickness'' of the collar. Section 6.14's
SU(3) lattice analysis finds A\_eff → 2.004 ± 0.012 as g² → 0.

Using g² = 4πα and defining the \textbf{normalization convention} A\_eff = 4π
(which differs from the lattice extrapolation by a factor of \textasciitilde2π; see
...
\emph{(Note: Using the MSSM unification prediction α\_s = 0.1166 instead would give
...
\emph{Normalization note:} The A\_eff = 4π convention gives t = 4π²α, a clean
relation used throughout. However, Section 6.14's lattice extrapolation
gives A\_eff → 2, not 4π ≈ 12.6. This factor-of-\textasciitilde6 discrepancy indicates
either: (1) the toy UV model's ``g²'' differs from the continuum MS-bar
convention by a factor of \textasciitilde2π, or (2) additional physics (spin-statistics,
...
the α\_s pixel-area extraction) require this normalization to be resolved
...
\textbf{Prediction (log 6 entropy deficit).} The Z₆ quotient produces a universal entropy deficit of exactly log₂ 6 bits in the edge-sector distribution, relative to the naive product group:

\textbf{log₂ 6 = 2.584962500721156\ldots{} bits}

This is a parameter-free constant fixed purely by the Z₆ identification.

\textbf{Derivation.} The quotient restricts each (τ, j) combination to a single residue class r ≡ n (mod 6). Define the residue sums

S\_r(t\_Y) := Σ\_k exp(−t\_Y(6k+r)²)

By Poisson summation, the relative deviation between residue sums is \textasciitilde2 exp(−π²/(36 t\_Y)). At M\_Z with t\_Y = 0.0186:
...
\textbf{Numerical result.} Computing the edge entropy S\_edge = H(p\_α) + ⟨log d\_α⟩:
...
\textbf{Why this is sharp.} The log₂ 6 entropy deficit is:
- Rigidly fixed by the Z₆ identification (not tunable)
- Independent of UV completion details
- Numerically precise to 10⁻⁶ bits at M\_Z
...
\subsubsection{6.19 Electroweak scale from dimensional transmutation}\label{electroweak-scale-from-dimensional-transmutation}
...
mass term m²\textbar H\textbar² is exactly such a gauge-invariant relevant scalar (Δ = 2 \textless{} 4).
...
\textbf{Comparison to measurement.} The measured Higgs VEV is v\_obs ≈ 246.2 GeV.
...
\textbf{Caveat.} This is not a genuine prediction. The assumption ``y\_t ≈ 1'' is an empirical fact; it's what makes the top quark special. The ``derivation'' restates observation rather than predicting it. A genuine prediction would derive y\_t ≈ 1 from first principles, which the framework does not do.

\subsubsection{6.21 Yukawa hierarchy from Z₆ defect suppression}\label{yukawa-hierarchy-from-zux2086-defect-suppression}
...
\textbf{The key observation.} The Z₆ entropy deficit is ΔS = ln 6 nats. Under MaxEnt logic, an insertion that requires resolving one unit of this defect carries a suppression factor:
...
(-1.1\% error); the reverse-engineered β\_EW = 4.00116 matches the integer 4
to 0.03\% precision.

\subsubsection{6.22 Higgs mass from critical surface constraint}\label{higgs-mass-from-critical-surface-constraint}
...
2. The refinement-stability constraint λ = β\_λ = 0 at M\_U
...
m\_t\^{}MS̄(M\_t) = y\_t(M\_t) · v / √2 ≈ (0.9192 × 246.22) / √2 ≈ 160.0 GeV
...
\subsubsection{6.23 Rigorous derivation chain: axioms to predictions}\label{rigorous-derivation-chain-axioms-to-predictions}
...
\rho\_C = \frac{\exp(-\sum_a \lambda_a O_a)}{Z(\lambda)}
...
\bar\{\ell\}\emph{\{\rm tot\} = \bar\{\ell\}}\{\rm SU(2)\}(t\_2) + \bar\{\ell\}\_\{\rm SU(3)\}(t\_3)
...
\frac{a_{\rm cell}}{\ell_p^2} = 4\bar\{\ell\}\_\{\rm tot\}(t\_2, t\_3)
...
\text{weight} \propto (d\_\{\rm SU3\} \cdot d\_\{\rm SU2\})\^{}2 \cdot p(R\_3, R\_2, y)
...
\Delta b\_\{\rm pred\} \approx (2.49, , 4.17, , 4.01)
...
- α̂⁻¹(M\_Z) = 127.951 ± 0.009
- ŝ²\_Z = sin²θ̂\_W(M\_Z) = 0.23122 ± 0.00004
- α\_s(M\_Z) = 0.1180 ± 0.0009

The piecewise running (SM below M\_S, edge-UV above) gives a 2×2 system for x = ln(M\_S/M\_Z) and y = ln(M\_U/M\_S). Solution:

\$\$

M\_S \approx 60 \text{ GeV}, \quad M\_U \approx 2.4 \times 10\^{}\{16\} \text{ GeV}, \quad \alpha\_U\^{}\{-1\} \approx 24.0
...
\textbf{Inputs:}
1. Pixel constant: a\_cell/ℓ\_p² = 1.631 (treat as fundamental)
2. One electroweak datum: α̂⁻¹(M\_Z) = 127.951

\textbf{Constraints:}
- Pixel: a\_cell/ℓ\_p² = 4(ℓ̄₂ + ℓ̄₃)
...
\sin\^{}2\hat{\theta}\_W(M\_Z) \approx 0.2310, \qquad \alpha\_s(M\_Z) \approx 0.1175
...
\subsubsection{8.1 Classification of results}\label{classification-of-results}
...
\item
  \textbf{Newton's constant formula}: G = a\_cell / (4 ℓ̄(t)) from edge entropy density (Section 5.4)
\item
...
\item
  \textbf{Edge entropy deficit ≈ log₂ 6 bits}: Heat-kernel law + Z₆ quotient
\item
...
\item
  \textbf{Weak mixing angle}: sin²θ\_W ≈ 0.2311 vs PDG ŝ²\_Z = 0.23122 ± 0.00006 (MS-bar, \textasciitilde2σ)
\item
...
\item
  \textbf{Dipolar radiation}: Model predicts none. Bound: δp̂₋₂ ∈ {[}−4×10⁻⁶, 2×10⁻⁵{]}
\item
...
\subsubsection{8.3 Novel testable predictions}\label{novel-testable-predictions}
...
f\_\{k,m\}(M,χ) = (m Ω\_H)/(2π) + (c³ g(χ))/(16π² GM) · ln(k) for k = 2, 3, 4, \ldots{}

After rescaling by remnant parameters, all events should stack at universal coordinates x\_k = ln(k)/(8π). This is checkable with public LIGO/Virgo data. Absence of coherent stacking at the predicted x\_k would falsify the log-integer area spectrum.
...
\textbf{Z₆ entropy fingerprint (Section 6.18).} The global gauge group quotient (SU(3)×SU(2)×U(1))/Z₆ produces a universal entropy deficit of exactly log₂ 6 ≈ 2.585 bits in the edge-sector distribution. This is a direct ``global-structure observable'': measuring edge-sector entropies of \textasciitilde6.6 bits instead of \textasciitilde4.0 bits would falsify the Z₆ quotient. The prediction is nearly scale-independent and requires no UV completion details.
...
\item
  \emph{Mass-independent fractional linewidth}: The intrinsic linewidth Γ/ΔE\_k ≈ 3-5\% is approximately independent of black hole mass. This is a sharp shape prediction constraining not just line positions but line profiles.
\item
...
\subsubsection{8.4 What is not predicted (gaps)}\label{what-is-not-predicted-gaps}
...
\item
  ...
  y\_f ∝ 6\^{}\{−n\_f\} with integer defect charges n\_f.~What remains: derive the
  integer charges from UV gluing/tensor geometry.
\item
  \textbf{β-function coefficients}: The Peter-Weyl second-index mechanism (Section 6.17)
  derives Δb ≈ (2.49, 4.38, 3.97) from the heat-kernel distribution at t\_U ≈ 1.64,
  matching MSSM targets to within 5\%. What remains: derive t\_U from group-theoretic
  principles and resolve the \textasciitilde5\% Δb₂ tension.
\item
  \textbf{Scheme matching}: The entanglement → MS̄ map uses Dynkin indices T(R) rather than dimensions, producing near-unity normalization. Remaining: fully derive the map from first principles.
\end{itemize}
...
\item
  ...
  the UV and which operator generates v with coefficient β\_EW = N\_c + 1 = 4.
